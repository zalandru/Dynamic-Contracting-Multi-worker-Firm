...
class JobSearch:
    ...
    def update(self,V,P,disp=False,type=0,relax=0):
        ...
        #Turning it all into tensors. Ideally everything would be tensors from the start tho
        #self.re_cst = torch.from_numpy(self.re_cst)
    def pe(self,e):
        """
    ...
    def pe_deriv(self,e):
        ...
        e = torch.minimum(e, self.e0)
        pe = self.x[1] *  self.x[2] * torch.abs(e - self.e_asy)** (self.x[2] -1.0)
        return pe
    ...
    def re(self,e):
        ...
        """
        e = torch.minimum(e, self.e0)
        return self.re_cst - self.x[0]*e + self.x[1]/(1+self.x[2])*(torch.abs( e - self.e_asy)** ( 1 + self.x[2]))
  ...
  #re_cst=p(v)(e0-e_asy) and the inside of the integral is the derivative of re wrt e.
    def ve(self,e):
        ...
        ve = self.re(e)/(1e-5+self.pe(e)) + torch.minimum(e,self.e0) #Andrei: uses re=p(v)(v-e) formula, rearranged to solve for v: v = re/(p(v)) + e
