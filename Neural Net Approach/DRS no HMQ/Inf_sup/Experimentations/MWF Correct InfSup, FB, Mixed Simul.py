...
#Fin Diff gradients
def get_batch_gradients_fd(states, value_model, num_y, range_tensor=None):
    """
    Computes per-sample gradient of E_{y'|y} V(y', s) for all y ∈ {0, 1, ..., num_y-1}
...
def get_expectation_gradients_fd(states, value_model, P_mat,  i, range_tensor=None, current_y=None):
    """
    Computes per-sample gradient of E_{y'|y} V(y', s) for all y ∈ {0, 1, ..., num_y-1}
...
def train(state_dim, value_net, sup_net, inf_net, optimizer_value, optimizer_sup, optimizer_inf, scheduler_value, scheduler_sup, scheduler_inf, foc_optimizer, bounds_processor, bounds_processor_sup, num_episodes=20, starting_points_per_iter=100, simulation_steps=5, 
    ...
    for episode in tqdm(range(num_episodes)):
        ...
        with torch.no_grad():
            ...
            states_rand, prod_states_rand, fut_states  = simulate(fut_states, sup_net, inf_net, bounds_processor, bounds_processor_sup, foc_optimizer.Z_trans_tensor, sim_steps_ep, random_paths) #This is the set of states we will use to train the value function.         #Now append future states to the states so that I can mini-batch them together.
        ...
        for mb in minibatches:    
            ...
            if ((batch_index) % minibatch_num)==0: #Inf Update, happens only once per episode
             ...
             if ((batch_index) % 3)==0: #Sup update
                ...
                states_eps = states + 1e-2 * tensor([0,0,1])
                predicted_grad_eps = get_batch_gradients(states, value_net, policies['hiring'].shape[1], range_tensor=bounds_processor.range)[i,prod_states.long(),:]
                mon_loss_grad = torch.relu( - (predicted_grad_eps[:,-1] - predicted_grad[:,-1])).pow(2).mean() #Monotonicity loss for the gradient
                smoothness_loss = (predicted_grad_eps[:,-1] - predicted_grad[:,-1]).pow(2).mean() #Smoothness loss for the gradient
                mon_loss_value = torch.relu( - predicted_grad[:,-1]).pow(2).mean() #The grad should be positive, so we penalize negative values
                tot_value_loss = (value_loss + λ * value_grad_loss) / (1+λ) + 1e-3 * mon_loss_grad (+ smoothness_loss + mon_loss_value) #Combine the losses
...
if __name__ == "__main__":
    ...
    K_n, K_v, HIDDEN_DIMS, learning_rate=[5e-4,3e-4,1e-4], weight_decay = [1e-2, 1e-2, 1e-2], pre_training_steps=200, num_epochs=num_episodes, minibatch_num=minibatch_num)
