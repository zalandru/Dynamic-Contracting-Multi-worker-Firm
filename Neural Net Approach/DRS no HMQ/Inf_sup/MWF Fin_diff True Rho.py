...
class FOCresidual:
    ...
    def future_values(self, states, prod_states, fut_states, omega, v_prime, hiring, value_net):
        ...
        EJ_star = ERho_star.unsqueeze(1) - omega * v_prime #This should all be policy shape
    ...
    def values(self, states, prod_states, EJ_star, v_prime, pc_star, re_star, hiring, future_grad):
    ...
    def FOC_loss(self, states, omega, hiring, pc, re, v_prime, future_grad):
        ...
        EJ_deriv = future_grad[:,1:K_n]# - omega * EW_star #Should it be like this??? Because, if I write 𝒫(y,n_k,̃ρ_k) = max_{v_k} J(y,n_k,v_k) + ̃ρ_k v_k, then v_k has no role to play in the derivative wrt n_k!
        size=self.bounds_processor.denormalize_size(states[:,:K_n])
...
def initialize(bounds_processor, state_dim, K_n, K_v, hidden_dims, learning_rate, weight_decay, pre_training_steps):
    #Initializations:
        ...
        for mb in minibatches:    
            ...
            if ((batch_index) % 4)==0: #Policy Update
                ...
                for it_inf in range(2):
                    ...
                    _, _, future_grad, _, _, _ = foc_optimizer.initiation(prod_states=prod_states, states = states, fut_states = fut_states, omega=omega, v_prime = v_prime, hiring=hiring, value_net=target_value_net)  #Note that I am using the target value here!!!          
                    inf_loss = nn.SmoothL1Loss(reduction='mean')(future_grad[:,K_n:], v_prime)
                ...
                #with torch.no_grad():
                omega = inf_net(states)['omega'][i,prod_states.long(),:] #not a target_inf_net so that the policy can see some of its impact
                ...
                FOC_wage_loss = nn.SmoothL1Loss(reduction='mean')(FOC_rho_resid, torch.zeros_like(FOC_rho_resid))
                FOC_hire_loss =  nn.SmoothL1Loss(reduction='mean')(FOC_hire_resid, torch.zeros_like(FOC_hire_resid))
                ...
                #for name, p in policy_net.named_parameters():
                    #p.register_hook(make_nan_hook(name))
                sup_loss.backward()
                #for name, p in policy_net.named_parameters():
                #    p.register_hook(make_nan_hook(name))
                ...
                assert (~torch.isnan(omega)).all() and (~torch.isnan(v_prime)).all() and (~torch.isnan(hiring)).all(), "policy returns NaN"
                EJ_star, v_prime, future_grad, re, pc, _ = foc_optimizer.initiation(prod_states=prod_states, states = states, fut_states = fut_states, omega=omega, v_prime = v_prime, hiring=hiring, value_net=target_value_net)  #Note that I am using the target value here!!!          
                target_values, target_grad = foc_optimizer.values(states=states, prod_states=prod_states, EJ_star=EJ_star.detach(), v_prime=v_prime.detach(), re_star=re.detach(), hiring=hiring.detach(), pc_star = pc.detach(), future_grad=future_grad.detach()) 
                ...
                value_loss = nn.SmoothL1Loss(reduction='mean')(pred_values, target_values)
                value_grad_loss = nn.SmoothL1Loss(reduction='mean')(predicted_grad, target_grad) #Get the value loss for the states in the minibatch
...
def evaluate_plot_policy(value_net, policy_net, inf_net, bounds_processor, num_samples=1000):
    ...
    # Plotting the results
    plt.figure(figsize=(10, 6))
    plt.scatter(states[:, 2].cpu(), omega[:,1,0].cpu(), alpha=0.5)
...
if __name__ == "__main__":
    ...
    HIDDEN_DIMS = [32,32,32]  # Basic architecture. Basically every paper has 2 inner layers, can make them wider though
    ...
    K_n, K_v, HIDDEN_DIMS, learning_rate=[1e-5,1e-5,1e-5], weight_decay = [1e-6, 1e-6, 1e-6], pre_training_steps=200)
    ...
    evaluate_plot_policy(trained_value, trained_policy, trained_inf, bounds_processor_wage, num_samples=1000)
