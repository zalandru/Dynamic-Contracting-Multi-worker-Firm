...
p = Parameters()
type = torch.float32
# Override tensor creation to include device
tensor = lambda data: torch.tensor(data, dtype=type, device=device)
...
class FOCresidual:
    ...
    def __init__(self, bounds_processor, bounds_processor_inf, K, p, cc):
        ...
        self.Z_trans_tensor = tensor(self.Z_trans_mat)
        # Value Function Setup
        self.w_grid = np.linspace(self.unemp_bf, self.fun_prod.max(), self.p.num_v )

        self.rho_grid=tensor(1/self.pref.utility_1d(self.w_grid))
        ...
        self.v_0 = tensor(self.v_grid[0])
        self.fun_prod = tensor(self.fun_prod)        
        self.simple_J = torch.zeros((self.p.num_z,self.p.num_v),device=device)
        self.simple_J= (self.fun_prod[:,ax] -self.pref.inv_utility(tensor(self.v_grid[ax,:]*(1-self.p.beta))))/ (1-self.p.beta)
        self.simple_Rho = self.simple_J + self.rho_grid[ax,:] * tensor(self.v_grid[ax,:])#We do indeed need to work with Rho here since we're taking W via its derivatives
        #Apply the matching function: take the simple function and consider its different values across v.
        self.prob_find_vx = self.p.alpha * np.power(1 - np.power(
            np.divide(self.p.kappa, np.maximum(self.simple_J[self.p.z_0-1, :].cpu(), 1.0)), self.p.sigma), 1/self.p.sigma)
        #Now get workers' probability to find a job while at some current value, as well as their return probabilities.


        if cc is None:
            self.js = JobSearchArray(device=self.device) #Andrei: note that for us this array will have only one element
    ...
    def values(self, states, prod_states, EJ_star, v_prime, pc, re, hiring, omega, future_grad):
        ...
        rho = states_d[:,K_n:]
        wages = torch.zeros_like(size) 
        wages[:,:1] = self.pref.inv_utility(self.v_0 - self.p.beta*((v_prime[:,:1]+re[:,:1])))
        wages[:,1:] = tensor(np.interp(rho.cpu().detach().numpy(),self.rho_grid.cpu(),self.w_grid))
        worker_values = torch.zeros_like(v_prime) #This is the value function for the worker, given the wages and the promised values
        if K_n > 2:
    ...
    def FOC_loss(self, states, omega, hiring, v_prime, future_grad):
        ...
        focs_rho = torch.zeros_like(v_prime)
        #Usual case first:
        if K_n > 2:
    ...
    def get_fut_size(self, states, hiring, v_prime):
        ...
        size = self.bounds_processor.denormalize_size(states[:,:K_n])
        fut_size = torch.zeros_like(size)
        fut_size[:,0] = hiring
        if K_n > 2:
...
def simulate(starting_states, sup_net, inf_net, bounds_processor, bounds_processor_inf, simulation_steps):
    ...
    S = torch.zeros(his_size_total, D, dtype=type, device=device) #Vector of all reached states
    Fut_states = torch.zeros(his_size_total, D, dtype=type, device=device) #Vector off all future states (we know them all besides y'). It's just policies for each state 
    ...
    P = torch.arange(S.shape[0], device=device) % p.num_z #Vector off all production states
    ...
    for t in range(simulation_steps):
        ...
        #Set up new states
        states = torch.zeros(his_size, D, dtype= type, device=device)
...
def initialize(bounds_processor, bounds_processor_inf, state_dim, K_n, K_v, hidden_dims, learning_rate, weight_decay, pre_training_steps, num_epochs, minibatch_num):
    ...
    value_net = ValueFunctionNN(state_dim, p.num_z, hidden_dims).to(device)
    sup_net = PolicyNN(state_dim, p.num_z, K_v, hidden_dims, cc).to(device)
    inf_net = infNN(state_dim, p.num_z, K_v, hidden_dims, cc).to(device)
...
def pre_training(optimizer_value,optimizer_sup, optimizer_inf, value_net,sup_net,inf_net, foc_optimizer,bounds_processor, bounds_processor_inf, K_n, K_v, pre_training_steps):
    rho_states = bounds_processor.normalize_rho(tensor(cc.rho_grid[:,ax]))
...
def train(state_dim, value_net, sup_net, inf_net, optimizer_value, optimizer_sup, optimizer_inf, foc_optimizer, bounds_processor, bounds_processor_inf, num_episodes=20, starting_points_per_iter=100, simulation_steps=5, 
    ...
    if use_saved_nets:
        ...
        value_net.load_state_dict(torch.load("trained_value_function.pt",map_location=device))
        sup_net.load_state_dict(torch.load("trained_sup_function.pt",map_location=device))
        inf_net.load_state_dict(torch.load("trained_inf_function.pt",map_location=device))
    #Initialize a target network
    target_value_net = copy.deepcopy(value_net).to(device)
    # Ensure it's not updated by the optimizer
    for param in target_value_net.parameters():
        param.requires_grad = False
    target_sup_net = copy.deepcopy(sup_net).to(device)
    for param in target_sup_net.parameters():
        param.requires_grad = False
    target_inf_net = copy.deepcopy(inf_net).to(device)
    ...
    for episode in tqdm(range(num_episodes)):
        ...
        state_start = torch.zeros(state_dim,dtype=type, device=device)
        state_start[0] = bounds_processor.normalize_dim(1,0) # 1 junior worker
        #Or if randomized. 
        states= torch.rand(starting_points_per_iter, state_dim,dtype=type, device=device) 
        ...
        comb_states = torch.zeros(states[pos_size,:].shape[0], state_dim, 3, dtype=type, device=device)
...
if __name__ == "__main__":
    ...
    K_n, K_v, HIDDEN_DIMS, learning_rate=[1e-3,3e-4,1e-4], weight_decay = [1e-4, 3e-5, 1e-5], pre_training_steps=0, num_epochs=num_episodes, minibatch_num=minibatch_num)
