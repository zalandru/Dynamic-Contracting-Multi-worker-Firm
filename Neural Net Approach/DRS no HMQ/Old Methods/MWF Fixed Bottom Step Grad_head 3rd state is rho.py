...
class FOCresidual:
    ...
    def future_values(self, states, prod_states, fut_states, rho_star, fut_values, hiring, value_net):
        ...
        future_grad = self.take_expectation(value_output['grads'],prod_states) #Here the 3rd dimenion of grads is n1 * EW_star, because the state space is (n0,n1,ρ)
    ...
    def values(self, states, prod_states, rho_star, EJ_star, EW_star, pc_star, re_star, hiring, future_grad):
        ...
        size = states[:, :K_n]
        rho = states[:,K_n:].squeeze(1)
        wages = torch.zeros(rho.shape[0],K_n) + 1e-2
        wages[s_pos,1] = tensor(np.interp((rho[s_pos]).detach().numpy(),self.rho_grid,self.w_grid),dtype = type)
        #wages[~s_pos,1] = 0
        ...
        grad[:,:K_n] = (self.fun_prod[prod_states.detach().long()] * self.production_1d_n(tot_size)).unsqueeze(1) - wages[:,:] + self.p.beta * pc_star * (future_grad[:,1:K_n] - rho_star * EW_star)# notice that future_grad is the same for juniors and seniors, because both end up becoming seniors
        grad[:,1:K_n] += worker_values * rho.unsqueeze(1)    
...
def simulate(starting_states, policy_net, bounds_processor, simulation_steps):
    ...
    for t in range(simulation_steps):
        ...
        states[:,K_n:] = wages.view(his_size,K_v) # state ρ_1
...
def train(state_dim, value_net, policy_net, optimizer_value, optimizer_policy, foc_optimizer, bounds_processor, num_episodes=20, starting_points_per_iter=100, simulation_steps=5, 
    ...
    for episode in tqdm(range(num_episodes)):
        ...
        for mb in minibatches:    
            ...
            EJ_star, EW_star, future_grad, re, pc, n_1 = foc_optimizer.initiation(prod_states=prod_states, states = states, fut_states = fut_states, rho_star=rho_star.detach(), values = values, hiring=hiring, value_net=target_value_net)  #Note that I am using the target value here!!!          
            ...
            if ((batch_index) % 4)==0: #Policy Update
                ...
                FOC_wage_loss = nn.MSELoss()(FOC_rho_resid, torch.zeros_like(FOC_rho_resid)) #I JUST DETACHED RHO_STAR HERE, SHOULD TRY AGAIN NOW!!!
                FOC_hire_loss =  nn.MSELoss()(FOC_hire_resid, torch.zeros_like(FOC_hire_resid))
                fut_states_adj = torch.cat((hiring.unsqueeze(1).detach(),n_1.detach(),rho_star),dim=-1)
                fut_states_adj = bounds_processor.normalize(fut_states_adj)
                value_output = target_value_net(fut_states_adj)['grads'] #This will be n1* EW_star, so I will multiply the other side by n1
                ERho_deriv_wrt_ω = foc_optimizer.take_expectation(value_output[...,K_n:],prod_states)
                fut_value_loss = nn.MSELoss()(values.detach()*n_1.detach(),ERho_deriv_wrt_ω)
                ...
                if (episode + 1) % (num_episodes/20) == 0 & batch_index % 4 == 0 & batch_index < 5:
                ...
                optimizer_value.zero_grad()
                target_values, target_grad = foc_optimizer.values(states=states, prod_states=prod_states, rho_star = rho_star.detach(), EJ_star=EJ_star.detach(), EW_star=EW_star.detach(), re_star=re.detach(), hiring=hiring.detach(), pc_star = pc.detach(), future_grad=future_grad.detach()) 
...
if __name__ == "__main__":
    ...
    value_net, policy_net, optimizer_value, optimizer_policy, foc_optimizer = initialize(bounds_processor_wage, STATE_DIM, K_n, K_v, HIDDEN_DIMS,
                                                                                         learning_rate=[1e-4,1e-5],pre_training_steps=0)
