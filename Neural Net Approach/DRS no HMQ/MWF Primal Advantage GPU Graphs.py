...
inductor_config.triton.cudagraph_trees = False  # <<< add this
...
def compile_or_fallback(module, device, mode="reduce-overhead"):
    try:
        if device.type == "cuda":
            ...
            return torch.compile(module, backend="inductor", mode=mode, fullgraph=True)
        else:
        ...
        return torch.compile(module, backend="aot_eager", mode=mode, fullgraph=True)

class StaircaseLRScheduler:
    def __init__(self, optimizer, total_steps, bins=8, base_lr=3e-4, min_lr=3e-5, mode="cosine"):
        self.opt = optimizer
    ...
    def _sched_val(self, step):
        t = min(step / self.total_steps, 1.0)
        if self.mode == "cosine":
            # Cosine anneal from base_lr to min_lr
            return self.min_lr + 0.5*(self.base_lr - self.min_lr)*(1 + math.cos(math.pi*t))
        elif self.mode == "exp":
            # Exponential decay to min_lr
            return max(self.min_lr, self.base_lr * (self.min_lr / self.base_lr) ** t)
        else:
            # Step every 1/bins of training
            k = int(t * self.bins)
            return max(self.min_lr, self.base_lr * (0.5 ** k))

    def lr_bin(self, step):
        # Map continuous LR to a staircase with 'bins' plateaus
        # plateau index 0..bins-1
        return min(int(step * self.bins / self.total_steps), self.bins - 1)

    def get_lr_for_step(self, step):
        # representative step for this bin’s LR (center of bin)
        b = self.lr_bin(step)
        center = int((b + 0.5) * (self.total_steps / self.bins))
        return self._sched_val(center)

    def maybe_update_and_recapture(self, step, set_lr_fn, recapture_fn):
        b = self.lr_bin(step)
        if b != self._last_bin:
            self._last_bin = b
            new_lr = self.get_lr_for_step(step)
            for pg in self.opt.param_groups:
                pg["lr"] = new_lr
            #set_lr_fn(new_lr)       # optional: log or mirror to another opt
            #recapture_fn()          # warmup+capture for the new LR
            return True, new_lr
        return False, self.opt.param_groups[0]["lr"]
...
class FOCresidual:
    ...
    def get_fut_size(self, states, v_prime_exp):
        ...
        return n_1, re, pc
    def simulate(self, starting_states, sup_net, value_net, bounds_processor, Z_trans_tensor, simulation_steps, random_paths=5, u_time=None):
        """
        ...
        cumprobs = Z_trans_tensor.cumsum(dim=1).to(torch.float32)  # [Z, Z]; ensure last col == 1.0
        ...
        for t in range(simulation_steps):
            ...
            #wages = torch.empty(states_d.shape[0], K_n, device=device, dtype=dtype)
            wages_sen = foc_optimizer.pref.inv_utility(v - p.beta * (v_prime_exp + re))
            wages_jun = self.pref.inv_utility(self.v_0 - self.p.beta*((v_prime_exp+re)))
            wages = torch.cat([wages_jun, wages_sen], dim=1)
            ...
            # in the time loop:
            if u_time is None:
                y_next = torch.multinomial(Z_trans_tensor[y_idx, :], 1).squeeze(1)
            else:
                u = u_time[t]                                # [N] uniforms
                cdf_rows = cumprobs[y_idx]                   # [N, Z]
                y_next = torch.searchsorted(cdf_rows, u.unsqueeze(1), right=False).squeeze(1)

            next_states = torch.cat([hiring.unsqueeze(1), n1, v_prime[iN, :, y_next]], dim=1)  # [N, D]
...
class CudaGraphStep:
    def __init__(self, sup_net, value_net, target_value_net, tau,
                 foc_optimizer, bounds_processor,
                 horizon, random_paths,
                 starting_points_per_iter, state_dim,
                 amp_device_type, amp_dtype,
                 opt_sup, opt_val, device):
        self.sup = sup_net
        self.val = value_net
        self.tgt = target_value_net
        self.tau = tau
        self.foc = foc_optimizer
        self.bp  = bounds_processor
        self.T   = horizon
        self.R   = random_paths
        self.SPI = starting_points_per_iter
        self.D   = state_dim
        ...
        # ---- static buffers (shapes must never change) ----
        N = self.SPI * self.R
        self.static_starting = torch.empty(self.SPI, self.D, device=device)
        self.static_u_time   = torch.empty(self.T, N, device=device, dtype=torch.float32)
        self.static_loss_val = torch.zeros((), device=device)
        self.static_loss_sup = torch.zeros((), device=device)

        self.g = torch.cuda.CUDAGraph()

    def _forward_losses(self):
        # one full step: simulate -> targets -> losses
        with torch.amp.autocast(device_type=self.amp_device_type, dtype=self.amp_dtype):
            S, P, G_flat, G_start, R_flat, V_last = self.foc.simulate(
                self.static_starting, self.sup, self.tgt, self.bp,
                self.foc.Z_trans_tensor, self.T, self.R, u_time=self.static_u_time
            )
            # value branch: fully detached copies
            S_v, P_v, G_v, R_v, Vv = S.detach(), P.detach(), G_flat.detach(), R_flat.detach(), V_last.detach()

            # build targets (your existing TD(λ) routine)
            tdlam_flat = td_lambda_targets_from_flat(
                R_flat=R_v, S_flat=S_v, P_flat=P_v, target_value_net=self.tgt,
                T=self.T, N=self.SPI * self.R, V_last_bootstrap=Vv,
                gamma=p.beta, lam=0.95
            )
            i = torch.arange(S_v.shape[0], device=S.device)
            pred_values = self.val(S_v)['values'][i, P_v.long()]

            μ = tdlam_flat.mean()
            σ = tdlam_flat.std(unbiased=False).clamp_min(1e-6)
            value_loss = torch.nn.functional.smooth_l1_loss(
                (pred_values - μ) / σ, (tdlam_flat - μ) / σ
            )

            #policy = self.sup(S)
            adv    = (G_flat - pred_values.detach())
            scale  = adv.std(unbiased=False).clamp_min(1e-6).detach()
            sup_loss = -(adv/scale).mean()

            # OPTIONAL: add small 1-step surrogate here, weight by alpha
            # sup_loss = sup_loss - alpha * J1.mean()

        return value_loss, sup_loss

    def warmup(self, iters=3):
        # allocate optimizer state & JIT caches
        for _ in range(iters):
            torch.compiler.cudagraph_mark_step_begin()
            self.opt_val.zero_grad(set_to_none=True)
            self.opt_sup.zero_grad(set_to_none=True)

            # === DEBUG: one-off eager+anomaly step to pinpoint the in-place line ===
            val_saved, sup_saved = self.val, self.sup
            try:
                # Use the original (eager) modules if compiled, else keep as-is
                self.val = getattr(self.val, "_orig_mod", self.val)
                self.sup = getattr(self.sup, "_orig_mod", self.sup)

                with torch.autograd.set_detect_anomaly(True):
                    vloss, sloss = self._forward_losses()  # calls simulate()
                    (vloss + sloss).backward()
            finally:
                self.val, self.sup = val_saved, sup_saved
            # === end debug block ===
    ...
    def capture(self):
        ...
        with torch.cuda.graph(self.g):
            vloss, sloss = self._forward_losses()
            vloss.backward()
            torch.nn.utils.clip_grad_norm_(self.val.parameters(), 1.0)
            self.opt_val.step()

            sloss.backward()
            torch.nn.utils.clip_grad_norm_(self.sup.parameters(), 1.0)
            self.opt_sup.step()
            soft_update(self.tgt, self.val, self.tau)  # inside the graph
            # keep scalars readable outside the graph
            self.static_loss_val.copy_(vloss)
            self.static_loss_sup.copy_(sloss)

    @torch.no_grad()
    def replay(self, starting_states):
        # 1) copy inputs (same shapes every time)
        self.static_starting.copy_(starting_states)
        # 2) create fresh randomness OUTSIDE the graph
        self.static_u_time.uniform_(0.0, 1.0)
        # 3) run the whole step on-GPU
        self.g.replay()
        # 4) return scalars if needed
        return float(self.static_loss_val), float(self.static_loss_sup)


def soft_update(target_net, source_net, tau: float):
    # target should NOT require grads
    with torch.no_grad():
        for t, s in zip(target_net.parameters(), source_net.parameters()):
            t.mul_(1.0 - tau)     # in-place: t = (1-τ) * t
            t.add_(s, alpha=tau)  # in-place: t += τ * s
def initialize(bounds_processor, state_dim, K_n, K_v, HIDDEN_DIMS_CRIT, HIDDEN_DIMS_POL, learning_rate, weight_decay, pre_training_steps, num_epochs, minibatch_num):
    ...
    scheduler_value = StaircaseLRScheduler(optimizer_value, total_steps=num_epochs,
                                 bins=10, base_lr=learning_rate[0], min_lr=learning_rate[0] * 1e-1, mode="cosine")
    scheduler_sup = StaircaseLRScheduler(optimizer_sup, total_steps=num_epochs,
                                 bins=10, base_lr=learning_rate[1], min_lr=learning_rate[1] * 1e-1, mode="cosine")
...
def train(state_dim, value_net, sup_net, optimizer_value, optimizer_sup, scheduler_value, scheduler_sup, foc_optimizer, bounds_processor, num_episodes=20, starting_points_per_iter=100, simulation_steps=5, 
    ...
    random_paths = 10000 
    step = CudaGraphStep(
    sup_net, value_net, target_value_net, 0.05,
    foc_optimizer, bounds_processor,
    horizon, random_paths,
    starting_points_per_iter, STATE_DIM,
    amp_device_type, amp_dtype,
    optimizer_sup, optimizer_value, device
    )
    def recapture_all():
        step.warmup(iters=2)   # short warmup to ensure state
        step.capture()

    def set_lr_value(lr,value=True):
        # Mirror LR to both if you want them tied; or use two schedulers
        if value:
            for pg in optimizer_value.param_groups: pg["lr"] = lr
        else:
            for pg in optimizer_sup.param_groups:   pg["lr"] = lr
    
    # Initial capture with initial LR
    set_lr_value(scheduler_value.get_lr_for_step(0),True)
    set_lr_value(scheduler_sup.get_lr_for_step(0),False)
    recapture_all()
    print("Training...")
    # Training loop
    for episode in tqdm(range(num_episodes)):
        ep = episode + 1
        # If it's time to move to a new LR plateau, update & recapture
        changed_v, lr_now_v = scheduler_value.maybe_update_and_recapture(
            ep, set_lr_fn=set_lr_value, recapture_fn=recapture_all
        )
        changed_s, lr_now_s = scheduler_sup.maybe_update_and_recapture(
            ep, set_lr_fn=set_lr_value, recapture_fn=recapture_all
        )
        if changed_v or changed_s:
            recapture_all()
        #Or if randomized. 
        ...
        torch.compiler.cudagraph_mark_step_begin()
        value_loss, sup_loss = step.replay(starting_states)
