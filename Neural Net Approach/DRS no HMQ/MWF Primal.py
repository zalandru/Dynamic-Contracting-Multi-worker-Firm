...
def pre_training(optimizer_value,optimizer_sup, value_net,sup_net, foc_optimizer,bounds_processor, K_n, K_v, pre_training_steps):
    ...
    for _ in range(pre_training_steps):
        ...
        predicted_grads_v = grads[:,:,K_n:] #These are gradients wrt rho_k forall k besides the bottom one
        ...
        future_value_loss = nn.MSELoss()(predicted_values, target_W[...,ax, ax])
...
def train(state_dim, value_net, sup_net, optimizer_value, optimizer_sup, scheduler_value, scheduler_sup, foc_optimizer, bounds_processor, num_episodes=20, starting_points_per_iter=100, simulation_steps=5, 
    ...
    # one-time setup
    λ_balancer_value = LambdaBalancer(init_lambda=0.5, ema=0.9, minv=0.1, maxv=10.0)
    λ_balancer_policy = LambdaBalancer(init_lambda=1.0, ema=0.9, minv=0.1, maxv=10.0)
    ...
    for episode in tqdm(range(num_episodes)):
        ...
        time_beg_sim = time()
        with torch.no_grad():
            states, prod_states, fut_states  = simulate_deterministic(states, target_sup_net, bounds_processor, sim_steps_deterministic) #This is the set of states we will use to train the value function.     
            #Now use those states to simulate further, but with random paths
            states_rand, prod_states_rand, _  = simulate(fut_states, target_sup_net, bounds_processor, foc_optimizer.Z_trans_tensor, sim_steps_ep, random_paths) #This is the set of states we will use to train the value function.
        time_end_sim = time()
        ...
        for mb in minibatches:    
            ...
            if (((batch_index) % 3)==0) & (ep>=100): #Policy update
                ...
                g_hire  = grad_l2_norm(FOC_hire_loss, value_net.parameters())
                g_v = grad_l2_norm(FOC_v_loss,  value_net.parameters())
                λ = λ_balancer_policy.update(g_hire.detach(), g_v.detach())  # smooth & clamp                
                sup_loss = FOC_hire_loss + λ * FOC_v_loss + 1e-2 * (mon_loss_values + mon_loss_hiring)
                sup_loss.backward()
                ...
                g_val  = grad_l2_norm(value_loss, value_net.parameters())
                g_grad = grad_l2_norm(value_grad_loss,  value_net.parameters())
                λ = λ_balancer_value.update(g_val.detach(), g_grad.detach())  # smooth & clamp
