...
class PlotCallback(BaseCallback):
    def __init__(self, plot_env, make_env, dim_index, grid_min, grid_max, grid_points=200, plot_every=200_000, y_idx=None):
        super().__init__()
        self.plot_env = plot_env                  # plain EconGymEnv (not vectorized)
        self.make_env = make_env                  # same factory you use for training
        self.dim_index = dim_index
        self.grid = np.linspace(grid_min, grid_max, grid_points)
        self.grid_points = grid_points
        self.y_idx = y_idx
        self.every = plot_every
        self.vn: VecNormalize | None = None

    # ---- small helpers (normalize obs, predict normalized V, then denormalize) ----
    def _vn(self) -> VecNormalize | None:
        if self.vn is None:
            self.vn = self.model.get_vec_normalize_env()  # live training VecNormalize
    ...
    def _denorm_value(self, v_norm: np.ndarray) -> np.ndarray:
        ...
        if vn is None or not getattr(vn, "norm_reward", True):
            # if you trained with norm_reward=False, values are already in true units
            return v_norm
    ...
    # ---- action->physical using Δ-param mapping with the OBS we’re plotting ----
    ...
    def _make_fig(self, obs_mat, vals_true, acts_phys, y_idx):
        # split your policy into components for plotting
    ...
    def _on_training_start(self) -> None:
        # ensure eval VecNormalize flags as well:
        vn = self._vn()
        if vn is not None:
            vn.training = False
...
def main():
    ...
    plot_cb = PlotCallback(
        ...
        plot_every=200_000,            # log every 200k steps
       y_idx=None,
    )
    ...
    model_sb3.learn(total_timesteps=total_timesteps, callback=[plot_cb, ckpt_cb], tb_log_name="run1")
