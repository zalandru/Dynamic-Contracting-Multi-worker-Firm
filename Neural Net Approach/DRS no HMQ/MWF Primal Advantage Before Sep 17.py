...
class FOCresidual:
    ...
    def simulate(self, starting_states, sup_net, value_net, bounds_processor, Z_trans_tensor, simulation_steps, random_paths=5):
        ...
        T, N, D = S_time.shape
        S_flat  = S_time.reshape(T * N, D)             # [T*N, D]
        P_flat  = P_time.reshape(T * N)                # [T*N]
        ...
        return S, P, G_flat, G_starting, R_flat, values_last_exp
...
def train(state_dim, value_net, sup_net, optimizer_value, optimizer_sup, scheduler_value, scheduler_sup, foc_optimizer, bounds_processor, num_episodes=20, starting_points_per_iter=100, simulation_steps=5, 
    ...
    for episode in tqdm(range(num_episodes)):
        ...
        horizon = np.floor(10 + 22 * (ep/num_episodes)).astype(int)
        if horizon <= 8:
            random_paths = np.minimum(p.num_z ** horizon, 2000).astype(int)
        else:
            random_paths = 2000 
        ...
        #Add a quick moonotonicity loss
        ...
        adv_flat = (tdlam_flat - pred_values.detach())
        scale = adv_flat.std(unbiased=False).clamp_min(1e-6)
        sup_loss = -(G_starting).mean() #Should this be normalized somehow?
        ...
        if (episode + 1) % (num_episodes // 20) == 0:
            eval_losses = evaluate_loss(value_net, sup_net, bounds_processor, foc_optimizer)
...
def evaluate_plot_precise(value_net, sup_net, init_size, bounds_processor, foc_optimizer):
    ...
    plt.plot(foc_optimizer.v_grid, values.detach(), label = "NN")   
    ...
    plt.subplot(1,3,3)
    plt.plot(foc_optimizer.v_grid, v_prime[:,0].detach().numpy(), label = "NN v_prime")    
    plt.plot(foc_optimizer.v_grid, hiring.detach().numpy(), label = "NN hiring")
...
def evaluate_loss(value_net, sup_net, bounds_processor, foc_optimizer,
    ...
    states = torch.rand(starting_points, bounds_processor.lower_bounds.shape[0], dtype=type)

    # Simulate environment
    S, P, G_flat, G_starting, R_flat, V_last_bootstrap = foc_optimizer.simulate(
        states, sup_net, value_net, bounds_processor, foc_optimizer.Z_trans_tensor,
