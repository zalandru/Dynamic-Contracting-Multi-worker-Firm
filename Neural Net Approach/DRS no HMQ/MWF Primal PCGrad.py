...
class StateBoundsProcessor:
    ...
    def normalize_size(self, states):
        """Scale states from [lower_bound, upper_bound] to [0, 1]"""
        return (states - self.lower_bounds[ax, :K_n]) / self.range[ax, :K_n]    #Note: this will need to be adjusted at multiple steps      
...
def simulate_deterministic(starting_states, sup_net, bounds_processor, simulation_steps):
    ...
    for t in range(simulation_steps):
        ...
        if t==0:
            ...
            sup = sup_net(states)
        hiring = sup['hiring'] [B_idx,curr_y_idx].repeat(p.num_z) #Shape [B]. 
...
def initialize(bounds_processor, state_dim, K_n, K_v, HIDDEN_DIMS_CRIT, HIDDEN_DIMS_POL, learning_rate, weight_decay, pre_training_steps, num_epochs, minibatch_num):
    ...
    optimizer_value_base = RangerOptimizer(
    ...
    optimizer_value = PCGrad(optimizer_value_base)
    optimizer_sup_base = RangerOptimizer(
        ...
        use_warmup=False,
        warmdown_active=False)
    optimizer_sup = PCGrad(optimizer_sup_base)
    
    # 1. Define your “pure” cosine‐restart scheduler:
    cosine = lr_scheduler.CosineAnnealingWarmRestarts(optimizer_value_base,T_0=10,T_mult=2,eta_min=learning_rate[0] * 1e-2)
    cosine_sup   = CosineAnnealingWarmRestarts(optimizer_sup_base,   T_0=10, T_mult=2, eta_min=learning_rate[1] * 1e-2)

    # 2. Define a short linear warm-up (e.g. over first 5 epochs):
    warmup_epochs = 1000
    warmup = lr_scheduler.LinearLR(optimizer_value_base,start_factor=1e-6,    # start at 1e-6×base_lr
    end_factor=1.0,       # ramp up to 1.0×base_lr 
    total_iters=warmup_epochs)

    warmup_epochs = 1000
    warmup_sup = lr_scheduler.LinearLR(optimizer_sup_base,start_factor=1e-6,    # start at 1e-6×base_lr
    end_factor=1.0,       # ramp up to 1.0×base_lr 
    total_iters=warmup_epochs)
    # 3. Chain them so that after warmup you switch to cosine:
    scheduler_value = lr_scheduler.SequentialLR(
    optimizer_value_base,
...
def pre_training(optimizer_value,optimizer_sup, value_net,sup_net, foc_optimizer,bounds_processor, K_n, K_v, pre_training_steps):
    ...
    crs_Rho = foc_optimizer.simple_J.t()
    ...
    for _ in range(pre_training_steps):
        ...
        value_loss = nn.MSELoss()(predicted_values, target_values) + nn.MSELoss()(predicted_grads_v, -target_omega[...,ax] * size[:,ax,1:]) +  nn.MSELoss()(predicted_grads_size, grad_size)
        ...
        sup_loss = future_value_loss + 1e-2 * mon_loss
        sup_loss.backward()
        optimizer_sup.step()
...
def train(state_dim, value_net, sup_net, optimizer_value, optimizer_sup, scheduler_value, scheduler_sup, foc_optimizer, bounds_processor, num_episodes=20, starting_points_per_iter=100, simulation_steps=5, 
    ...
    for episode in tqdm(range(num_episodes)):
        ...
        sim_steps_ep = np.floor(2 + 4 * (ep/num_episodes)).astype(int)
        ...
        time_end_sim = time()
        #Add just a few random states. I'm not simulating ahead, but just adding a couple of outliers
        states_basic = torch.rand(3, state_dim,dtype=type) 
        prod_states_basic = tensor([p.z_0-2,p.z_0-1,p.z_0])
        #states_basic[:,:K_n] = bounds_processor.normalize_size([2,2].repeat(3))
        states = torch.cat((states, states_rand,states_basic), dim=0) #Now we have all the states together

        prod_states = torch.cat((prod_states, prod_states_rand,prod_states_basic), dim=0)
        ...
        for mb in minibatches:    
            ...
            if (((batch_index) % 3)==0) & (ep>=50): #Policy update
                ...
                mon_loss_values = torch.relu( - (policies_eps['values'] - policies['values'])/1e-2).abs().mean()
                mon_loss_hiring = torch.relu( (policies_eps['hiring'] - policies['hiring']) / 1e-2).abs().mean()
                #sup_loss =  FOC_hire_loss + FOC_v_loss * K_v * p.num_z + 1e-2 * (mon_loss_values + mon_loss_hiring)
                λ = 3 * (torch.exp(tensor(ep/num_episodes)) - 1)
                λ_adv = torch.max(0,1 - (2/3) * torch.exp(tensor(ep/num_episodes)))
                losses= [λ_adv * advantage,λ * FOC_hire_loss, λ * FOC_v_loss * K_v * p.num_z, 1e-3 * mon_loss_values, 1e-3 * mon_loss_hiring]
                optimizer_sup.pc_backward(losses)
                ...
                #target_grad    *= scale
                assert (not torch.isnan(predicted_grad).any()) and (not torch.isnan(pred_values).any()) and (not torch.isnan(target_values).any()) and (not torch.isnan(target_grad).any()), "NaN in FOC_hire_loss"
                ...
                mon_loss_grad = torch.relu( (predicted_grad_eps[:,-1] - predicted_grad[:,-1]) / 1e-2).abs().mean() #Monotonicity loss for the gradient
                #smoothness_loss =((predicted_grad_eps[:,-1] - predicted_grad[:,-1])/1e-2).pow(2).mean() #Smoothness loss for the gradient
                mon_loss_value = torch.relu( predicted_grad[:,-1]).abs().mean() #The grad should be negative, so we penalize negative values
                ...
                losses = [value_loss,λ * value_grad_loss, 1e-2 * mon_loss_grad, 1e-2 * mon_loss_value]
                #assert len(losses) == num_tasks
                optimizer_value.pc_backward(losses)
...
if __name__ == "__main__":
    ...
    K_n, K_v, HIDDEN_DIMS_CRIT, HIDDEN_DIMS_POL, learning_rate=learning_rate, weight_decay = [5e-3, 5e-5], pre_training_steps=0, num_epochs=num_episodes, minibatch_num=minibatch_num)
