...
class PolicyNN(nn.Module):
    ...
    """
    def __init__(self, state_dim, num_y, K_v, hidden_dims=[40, 30, 20, 10], bounds_processor=None):
        ...
        self.wage_head = nn.Sequential(
            ...
            ReLUPlusEps(eps=bounds_processor.lower_bounds[0])
        )
...
class FOCresidual:
    ...
    def __init__(self, bounds_processor, K, p, cc):
        ...
        self.rho_normalized = self.bounds_processor.normalize(self.rho_grid).unsqueeze(1).requires_grad_(True)
    ...
    def future_values(self, prod_states, fut_states, rho_star, hiring, value_net):
        ...
        fut_size = fut_states[:,1:K_n]
        future_grad = get_expectation_gradients(torch.cat((hiring.unsqueeze(1),fut_size,self.bounds_processor.normalize(rho_star)),dim=-1), value_net, self.Z_trans_tensor, range_tensor=self.bounds_processor.range,current_y = prod_states)
        ...
        EJ_star = ERho_star.unsqueeze(1) - rho_star * EW_star #This should all be policy shape
    ...
    def values(self, states, prod_states, EJ_star, EW_star, re_star, hiring):
        """Compute the value function for the given states and policy"""
        rho = self.bounds_processor.denormalize(states[:,K_n:])
        wages = tensor(np.interp(rho.detach().numpy(),self.rho_grid,self.w_grid),dtype = type)
        worker_values = self.pref.utility(wages) + self.p.beta * (EW_star + re_star)
        wage_jun = self.pref.inv_utility(self.v_0 - self.p.beta*((EW_star+re_star)))
        tot_size = 0
        tot_wage = 0
        for k in range(K_n):
            tot_size += states[:,k]
            if k>0:
                tot_wage += states[:,k] * wages[:,k-1]
        values = self.fun_prod[prod_states.detach().long()] * self.production(tot_size) - self.p.hire_c * hiring - \
            states[:,0] * wage_jun.squeeze(1) - states[:,1] * wages.squeeze(1) + states[:,1] * rho * worker_values.squeeze(1)
        #values = tensor(values, dtype=type) #Convert to tensor
        values+= self.p.beta * EJ_star.squeeze(1)
    ...
    def FOC_loss(self, states, rho_star, pc, re, EW_star, future_grad):
        ...
        focs_rho_2ndpart = - size[:, 1:] * self.bounds_processor.denormalize(states[:, K_n:]) -   size[:,:1]  / inv_utility_1d                         
        focs_rho = rho_star - EJ_deriv[:,1:] * (log_diff / self.deriv_eps) #first dim is productivity, second is future marg utility, third is today's margial utility
    ...
    def initiation(self, prod_states, fut_states, rho_star, hiring, value_net):
        _, EJ_star, EW_star, future_grad = self.future_values(prod_states, fut_states, rho_star, hiring, value_net)
...
def simulate(starting_states, policy_net, bounds_processor, simulation_steps):
    ...
    for t in range(simulation_steps):
        ...
        tot_size = states[:,0]+states[:,1]
        #Set up new states
        states = torch.zeros(his_size, D, dtype= type)
        #STUPID MISTAKE, BUT A HARD FIX. NEED PREVIOUS STATES, BUT THOSE ARE ALREADY GONE+ OF DIFF SIZE
...
def initialize(bounds_processor, state_dim, K_n, K_v, hidden_dims, learning_rate, pre_training_steps):
    ...
    policy_net = PolicyNN(state_dim, p.num_z, K_v, hidden_dims, bounds_processor)
    ...
    for episode in tqdm(range(num_episodes)):
        ...
        for mb in minibatches:    
            ...
            if ((batch_index) % 4)==0: #Policy Update
                FOC_rho_resid,FOC_hire_resid = foc_optimizer.FOC_loss(states=states, rho_star=rho_star, pc=pc, re=re, EW_star=EW_star, future_grad=future_grad)
                FOC_loss = nn.MSELoss()(FOC_rho_resid, torch.zeros_like(FOC_rho_resid)) + nn.MSELoss()(FOC_hire_resid, torch.zeros_like(FOC_hire_resid))
                ...
                target_values, target_W = foc_optimizer.values(states=states, prod_states=prod_states, EJ_star=EJ_star, EW_star=EW_star, re_star=re, hiring=hiring)
                pred_values = value_net(states)
                pred_values = pred_values[i,prod_states.long()] #Get the values for the states in the minibatch
                predicted_W = get_batch_gradients(states, value_net,  range_tensor=bounds_processor.range)[:,:,K_n:]
