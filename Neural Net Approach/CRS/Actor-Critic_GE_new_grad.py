...
def get_expectation_gradients(states, value_model, P_mat, range_tensor=None, current_y=None):
    ...
    for b in range(B):
        s = states[b].unsqueeze(0)  # [1, D]
        s.requires_grad_()  # Make sure it's tracked properly
        def f(input_s):
            return value_model(input_s.unsqueeze(0)).squeeze(0)  # [num_y]

        jac = torch.autograd.functional.jacobian(f, s.squeeze(0))  # [num_y, D]
        if range_tensor is not None:
            jac = jac / range_tensor
        grads.append(jac)

    jacobian = torch.stack(grads, dim=0)  # [B, num_y, D]
