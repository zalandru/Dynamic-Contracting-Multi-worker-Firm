...
class ValueFunctionNN(nn.Module):
    """Neural network that predicts both V(y';s) and its ∇ₛ-derivative."""
    ...
    def forward(self, x: torch.Tensor):
        ...
        Args:
            x: [B, state_dim]
        Returns:
            values: torch.Tensor of shape [B, num_y]
            grads:  torch.Tensor of shape [B, num_y, state_dim]
        """
        B = x.size(0)
...
class FOCresidual:
    ...
    def future_values(self, prod_states, policies, value_net):
        ...
        value_net_output = value_net(policies_norm)
        EW = value_net_output['grads'][:,:,0] #This is the gradient of the value function with respect to the state, for the first productivity state
        EW_star= torch.einsum("by,zy->bz", EW, self.Z_trans_tensor)
        EW_star = EW_star[i,prod_states.long()]
        V_all = value_net_output['values']   
    ...
    def values(self, states, prod_states, EJ_star, EW_star, pc_star, re_star):
        ...
        values = self.fun_prod[prod_states.detach().long()] - tensor(wages,dtype=type) + states.squeeze(1) * worker_values + self.p.beta * pc_star * EJ_star 
...
def simulate(starting_states, policy_net, value_net, bounds_processor, simulation_steps):
    ...
    for t in range(simulation_steps):
        ...
        V[his_start:his_end] = value_net(states)['values'].view(-1)
...
def evaluate_value_function(model, policy, p, lower_bounds,upper_bounds,cc,cc_Rho,cc_W,rho_star):
    ...
    values = model(test_states)['values'][:,p.z_0-1]
