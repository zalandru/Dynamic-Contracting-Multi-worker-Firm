...
class FOCresidual:
    ...
    def future_values(self, prod_states, policies, value_net):
        ...
        #policies_norm = policies 
        EW_star = get_expectation_gradients(policies_norm.requires_grad_(True), value_net, self.Z_trans_tensor, range_tensor=self.bounds_processor.range,current_y = prod_states)[:,0]

        EW_star_graph = get_expectation_gradients(policies_norm.detach().requires_grad_(True), value_net, self.Z_trans_tensor, range_tensor=self.bounds_processor.range,current_y = prod_states)[:,0]
        ...
        return ERho_star, EJ_star, EW_star, EW_star_graph    
    ...
    def initiation(self, prod_states, policies, value_net):
        _, EJ_star, EW_star, EW_star_graph = self.future_values(prod_states, policies, value_net)
        _, re, pc = self.getWorkerDecisions(EW_star)

        return EJ_star, EW_star, EW_star_graph, re, pc        
    ...
    for episode in tqdm(range(num_episodes)):
        ...
        for minibatch_X in minibatches:    
            ...
            if ((batch_index) % 4)==0:
                ...
                EJ_star, EW_star, EW_star_graph, re, pc = foc_optimizer.initiation(prod_states=minibatch_X[:,-2], policies=policies.unsqueeze(1), value_net=target_value_net)  #Note that I am using the target value here!!!      
                ...
                #print("EW_star_graph requires_grad:", EW_star_graph.requires_grad)
                #print("EW_star equals graph:", torch.allclose(EW_star, EW_star_graph))
                #print("Surrogate diff mean:", (EW_star_graph - EW_star.detach()).abs().mean().item())
                surrogate_loss = nn.MSELoss()(EW_star_graph, EW_star.detach())
                #FOC_loss #Get the total loss for the states in the minibatch
                policy_loss = FOC_loss + surrogate_loss
                policy_loss.backward()
                total_norm = 0
                for name, p in policy_net.named_parameters():
                    if p.grad is not None:
                        norm = p.grad.norm().item()
                        #print(f"[{name}] grad norm = {norm:.3e}")
                        total_norm += norm
                    else:
                        print(f"[{name}] grad is None")
                #print(f"[Total grad norm for policy_net: {total_norm:.3e}]")
                #print("Mean EW_star grad:", EW_star.abs().mean().item())
                #print("FOC residual mean:", FOC_resid.abs().mean().item())
                optimizer_policy.step()
                optimizer_policy.zero_grad()
            else:
                #with torch.no_grad():
                policies = policy_net(minibatch_X[:,:-2].requires_grad_(True))[i,minibatch_X[:,-2].long()]
                EJ_star, EW_star, _, re, pc = foc_optimizer.initiation(prod_states=minibatch_X[:,-2], policies=policies.unsqueeze(1), value_net=target_value_net)  #Note that I am using the target value here!!!      
        ...
        if (episode + 1) % (num_episodes/10) == 0 or episode == 0:
            print(f"Iteration {episode + 1}, Value Loss: {value_loss.item():.6f}, FOC_loss: {FOC_loss.item():.6f}, surr_loss: {surrogate_loss.item():.6f}" )
