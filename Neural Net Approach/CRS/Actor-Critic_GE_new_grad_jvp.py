...
def get_expectation_gradients(
    states: torch.Tensor,                # [B, D], must require_grad=True
    value_model: torch.nn.Module,        # maps [1, D] → [1, num_y]
    P_mat: torch.Tensor,                 # [num_y, num_y]
    range_tensor: torch.Tensor = None,   # [D] or None
    current_y: torch.Tensor = None       # [B], long
) -> torch.Tensor:
    """
    Computes, for each b in batch:
      ∇_s [ sum_{y'} P_mat[y', current_y[b]] * V(y'; s) ]
    in one backward pass per sample, and preserves the graph
    all the way back into `states`.
    """
    assert current_y is not None, "need current_y to pick the right column of P_mat"
    B, D = states.shape

    # Make sure states will track gradients
    states = states.requires_grad_()

    outs = []
    for b in range(B):
        s = states[b]                           # [D]
        V = value_model(s.unsqueeze(0)).squeeze(0)   # [num_y]
        w = P_mat[:, current_y[b].long()]              # [num_y]
        # form the scalar expectation
        exp = torch.dot(V, w)                   # scalar
        # get ∇_s exp in graph
        (g,) = torch.autograd.grad(exp, s, create_graph=True)
        if range_tensor is not None:
            g = g / range_tensor
        outs.append(g)
