...
# If these are unstable, your simulation logic is flawed
    ...
    for episode in tqdm(range(num_episodes)):
        ...
        batch_groups = list(zip(*[iter(minibatches)]*4))
        batch_index = 0
        #Vectorizing the common computations
        for group in batch_groups:
        # First compute all necessary outputs
            wrapper_outputs = []
            for mb in group:
        #    foc_resid, V_targs, W_targs = foc_optimizer.wrapper(
        #    states=mb.states,
        #    prod_states=mb.prod_states,
        #    policies=policy_net(mb.states),
        #    value_net=value_net
        #)
                i = torch.arange(mb.shape[0])
                policies = policy_net(mb[:,:-2].requires_grad_(True))[i,mb[:,-2].long()]
                EJ_star, EW_star, re, pc = foc_optimizer.initiation(prod_states=mb[:,-2], policies=policies.unsqueeze(1), value_net=value_net.detach())
                wrapper_outputs.append((mb, i, policies, EJ_star, EW_star, re, pc))
        
            for mb, i,policies, EJ_star, EW_star, _, pc in wrapper_outputs[:len(group)//4]: #Policy update
                FOC_resid = foc_optimizer.FOC_loss(states=mb[:,:-2], policies=policies.unsqueeze(1), pc=pc, EJ_star=EJ_star, EW_star=EW_star)
                FOC_loss = nn.MSELoss()(FOC_resid, torch.zeros_like(FOC_resid))
                loss = FOC_loss #Get the total loss for the states in the minibatch
                loss.backward()
                optimizer_policy.step()
                optimizer_policy.zero_grad()
            for mb, i, policies, EJ_star, EW_star, re, pc in wrapper_outputs[len(group)//4:]: #Value update            
                target_values, target_W = foc_optimizer.values(states=mb[:,:-2], prod_states=mb[:,-2], EJ_star=EJ_star, EW_star=EW_star, pc_star=pc, re_star=re)
                pred_values = value_net(mb[:,:-2])
                pred_values = pred_values[i,mb[:,-2].long()] #Get the values for the states in the minibatch
                predicted_W = get_batch_gradients(mb[:,:-2], value_net,  range_tensor=bounds_processor.range)[:,:,0]
                predicted_W = predicted_W[i,mb[:,-2].long()] #Get the values for the states in the minibatch
