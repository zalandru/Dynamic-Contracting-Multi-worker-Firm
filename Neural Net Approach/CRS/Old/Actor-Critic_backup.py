...
# If these are unstable, your simulation logic is flawed
    ...
    for episode in tqdm(range(num_episodes)):
        ...
        for minibatch_X in minibatches:    
            ...
            policies = policy_net(minibatch_X[:,:-2].requires_grad_(True))[i,minibatch_X[:,-2].long()]#Do
            FOC_resid, target_values, target_W = foc_optimizer.wrapper(states=minibatch_X[:,:-2], prod_states=minibatch_X[:,-2], policies=policies.unsqueeze(1), value_net=value_net) #Get the FOC residuals for the states in the minibatch
            ...
            #Entropy loss
            #entropy = - (policies.softmax(dim=1) * policies.log_softmax(dim=1)).sum(dim=1)  # [B]
            #entropy_loss = -0.01 * entropy.mean()  # Encourage exploration
            loss = value_loss + λ * FOC_loss# + entropy_loss
            loss.backward()
