...
class PolicyNN(nn.Module):
    ...
    """
    def __init__(self, state_dim, num_y, hidden_dims=[40, 30, 20, 10], cc=None):
        ...
        self.value_head = nn.Sequential(
            nn.Linear(input_dim, num_y * num_y), #Policy v'_{y',k}(y), for each [y,k,y']
            nn.Softplus()
        )
    ...
    def _init_weights(self):
        ...
        for seq in (self.value_head, self.sep_head):
            for layer in seq:
                if isinstance(layer, nn.Linear):
                    nn.init.xavier_uniform_(layer.weight, gain=1.0)
                    nn.init.constant_(layer.bias, 0.01) #keep in mind, I may not want this for tanh!!!
    ...
    def forward(self, x):
        ...
        values = values_flat.view(B, self.num_y, self.num_y)  # [B, num_y, num_Kv]
        ...
        # sep probabilities: [B, num_y]
        sep = (self.sep_head(features) + 1) / 2  #scaled tanh to [0,1]      # [B, num_y]
...
class FOCresidual:
    ...
    def __init__(self, bounds_processor, p, cc):
        ...
        from primitives_CRS import Preferences as pref_crs
        self.pref = pref_crs(input_param=self.p)
        ...
        self.U = tensor(self.unemp_bf / (1 - self.p.beta),dtype=type) #really we should be updating this
        # Transition matrices
        ...
        self.simple_J= (self.fun_prod[:,ax, ax]*self.qual_prod -self.pref.inv_utility(self.v_grid[ax,:,ax]*(1-self.p.beta)))/ (1-self.p.beta)
    ...
    def take_expectation(self, x, i,prod_states,v_prime=None):    
        if v_prime is not None:
         ...
         if x.ndim == 2:  # If x is [B, D]
            ...
            x = x[i,prod_states.long(),:]            
        return x
    def qual_production(self,q):
        return 1 * q  + self.p.prod_q * (1 - q)
    ...
    def simulate(self, starting_states, sup_net, value_net, bounds_processor, Z_trans_tensor, simulation_steps, random_paths=5):
        ...
        remaining    = torch.zeros((N, simulation_steps), device=device, dtype=dtype)
        ...
        for t in range(simulation_steps):
            ...
            pol      = sup_net(states)
            sep   = pol['sep'][iN, y_idx] #* self.bounds_processor.upper_bounds[0]                          # [N]
            v_prime  = pol['values'][iN, y_idx, :] * self.bounds_processor.upper_bounds[0]                   # [N, K_v, Z]
            ...
            v = states_d[:, 0]
            qual = states_d[:, 1]
            wage = foc_optimizer.pref.inv_utility(v - p.beta * (self.U * sep + (1 - sep)* (v_prime_exp + re)))
            #Getting reward for this state
            r_t = self.fun_prod[y_idx.detach().long()] * self.qual_production(qual) - wage
            rewards[:, t] = r_t
            remaining[:,t] = (1-sep) * pc
            ...
            next_states[:, 0]  = v_prime[iN, y_next]                # [N, K_v]
            next_states[:, 1] = torch.minimum(qual / (1 - sep),tensor(1,dtype=type))
            # each child gets v′(·, y′)
  
            # safety & normalization
            assert not torch.isnan(next_states).any(), "NaN in simulated states"
            assert torch.all(next_states[:, :] >= 0), "Negative v′ components in states"
        ...
        for t in range(simulation_steps - 1, -1, -1):
            running = rewards[:, t] + self.p.beta * remaining[:,t] * running #we discount by the number of remaining workers
            G[:, t] = running
...
def initialize(bounds_processor, state_dim, HIDDEN_DIMS_CRIT, HIDDEN_DIMS_POL, learning_rate, weight_decay, pre_training_steps, num_epochs, minibatch_num):
    #Initializations:
    # Initialize the neural networks
    value_net = ValueFunctionNN(state_dim, p.num_z, HIDDEN_DIMS_CRIT)
    sup_net = PolicyNN(state_dim, p.num_z, HIDDEN_DIMS_POL, cc)  
    #Compile the nets
    ...
    foc_optimizer = FOCresidual(bounds_processor, p=p, cc=None)    
...
def train(state_dim, value_net, sup_net, optimizer_value, optimizer_sup, scheduler_value, scheduler_sup, foc_optimizer, bounds_processor, num_episodes=20, starting_points_per_iter=100, simulation_steps=5, 
    ...
    delta_v =tensor([1e-3,0],device=device)

    state_start = torch.zeros(state_dim,dtype=type, device=device)#.requires_grad_(True)
    state_start[1] = bounds_processor.normalize_dim(p.z_0,1) # Lowest quality
    #state_start[0] = torch.randn()
    horizon = 32
    random_paths = 50 
    print("Training...")
    # Training loop
    for episode in tqdm(range(num_episodes)):
        ...
        starting_states[:,1] = state_start[1] #just making sure the starting quality is the bottom one
...
def evaluate_plot_precise(value_net, sup_net, init_size, bounds_processor, foc_optimizer):
    ...
    Args:
        ...
        state_dim: Dimension of state space
    """
    test_states = bounds_processor.normalize_dim(tensor(foc_optimizer.v_grid, dtype = type, device=device),0).unsqueeze(1)
    
    #size = torch.zeros(test_states.shape[0], 2, dtype = type, device=device)
    ...
    test_states = torch.cat((test_states,tensor([p.q_0],dtype=type).repeat(p.num_v).unsqueeze(1)),dim=1)
    # Evaluate model
    values = value_net(test_states)['values'][:,p.z_0-1]
    #Evaluate policies
    policy = sup_net(test_states)
    v_prime = policy['values'][:,p.z_0-1,p.z_0-1] * bounds_processor.upper_bounds[0]
    sep = policy['sep'][:,p.z_0-1] 

    W=get_batch_gradients(test_states, value_net,  num_y = foc_optimizer.p.num_z, range_tensor=bounds_processor.range.cpu())[:,p.z_0-1,0].detach().numpy()
    # WAIT!!! THIS IS NOT W ANYMORE!!! THIS IS -1/u'(w) aka -w!!!
    ...
    plt.subplot(1,3,3)
    plt.plot(foc_optimizer.v_grid, v_prime.detach().cpu().numpy(), label = "NN v_prime")    
    plt.plot(foc_optimizer.v_grid, sep.detach().cpu().numpy(), label = "NN sep")
...
if __name__ == "__main__":
    ...
    #Number of states
    STATE_DIM = 2 # + K_q #Discrete prod-ty y as multiple outputs
    ACTION_DIM = 1 # + K_n  # omega + hiring + separations. 
    ...
    LOWER_BOUNDS = [cc.v_grid[0], p.z_0] # The state space is (y,v,z).
    UPPER_BOUNDS = [1.1 * cc.v_grid[-1], 1]
    ...
    value_net, sup_net, optimizer_value, optimizer_sup, scheduler_value, scheduler_sup, foc_optimizer = initialize(bounds_processor,  STATE_DIM, 
     HIDDEN_DIMS_CRIT, HIDDEN_DIMS_POL, learning_rate=learning_rate, weight_decay = [1e-4, 3e-4], pre_training_steps=0, num_epochs=num_episodes, minibatch_num=minibatch_num)
