\section{Model}
I now present a model of a frictional labor market that features firms signing workers of varying match quality to dynamics contracts. This model is meant to capture the heterogeneity in wage passthrough and layoffs across firms and tenure documented in the data.
...
\textbf{Discretizing the Problem} \label{subsection:discrete} \\
In the current formulation, this problem is intractable as it involves a probability distribution, an uncountably infinitely-dimensional object, in the state-space. I show that this state space can be discretized, thus bringing it ``down" to countably infinite states.\\
...
I show how my model achieves the connection between layoffs and wage passthrough, as well as the heterogeneity of both across tenure. \\
\textbf{Wage Growth}
...
I start by characterizing the wage growth.
...
% \[ w'_{\bar{k}+1}-w'_{k+1}<w_{\bar{k}}-w_k \; \forall k\neq\bar{K}\]
 ...
 \end{definition}
The target wage captures the point where the marginal profit of a cohort $k$ reaches zero, thus making the firm indifferent between keeping or losing these workers to poaching ( it may still want to lose the low quality matches within the cohort and keep the high quality ones). The target wage is crucial for understanding the wage movements between cohorts:
\begin{proposition} \label{prop:targetwage}
 For each firm state $(y,\{n_k,v_k,z_k\})$ and cohort $k$ there exists a target wage $w^*_k$. 
  \begin{enumerate}
    ...
    \item Wage growth is faster the further wage is from the target wage:
    \[|w_k-w^*_k|\geq|w_{k'}-w^*_{k'}|\: \implies |w'_{k+1}-w_k|\geq|w'_{k'+1}-w'_{k'}| \]
    Moreover, to first-order:
...
\end{proof}
At any state and cohort, the wage adjusts towards $w^*_k$. I show how the target wage is affected by layoffs to connect layoffs and wage passthrough. Note that layoffs are the only tool the firm has to affect the quality within the cohort. Thus, following point 4 of the Proposition~\ref{prop:targetwage} it is also the only way to differentially change the target wage across cohorts.

%\begin{corollary}
%  Let $K_s\equiv \{k\leq K|s_k>0\}$. In states where $K_s$ is non-empty, target wage rises the most in the cohorts that suffered the most layoffs:
%  \[w^*_{k+1}-w^*_k > w^*_{k'+1}-w^*_{k'} \: \forall k\in K_s,k' \: s.t. s_{k'}<s_k\]
%\end{corollary}
%\textcolor{red}{Is it safer (easier to prove) to reformulate this in the first-order sense? That is, to first-order, cohorts that suffered the most layoffs will have the highest increase in the target wage. Not sure though.} \\
After the layoffs, the remaining workers are now of higher quality, thus the firm has a higher incentive to keep these workers. The immediate impact of this, combined with Point 2 of Proposition~\ref{prop:targetwage}, is that survivors of the layoffs will experience higher wage growth than the cohorts that didn't undergo any layoffs. One exception is the case where some cohorts were very far from their target wage, but happened to not get laid off. While possible, looking into the patterns of optimal layoffs suggests this may be unlikely:
...
Lower value cohorts are more subject to layoffs, no matter their quality (if below 1). With the exception of large quality differences, lower value cohorts will also be the ones farthest away from their target wage and thus, after layoffs, will experience the highest wage growth. \\
Finally, to apply this result for the heterogeneity in layoffs and passthrough across tenure, note that higher tenure workers will generally have higher promised value than junior workers due to wage backloading, as described in result 1 of Proposition~\ref{prop:targetwage}. Therefore, junior workers, due to being owed less, are generally more subject to layoffs than seniors and, due to the pushup effect of layoffs on the target wage, should generally suffer smaller wage cuts (if any) than seniors.