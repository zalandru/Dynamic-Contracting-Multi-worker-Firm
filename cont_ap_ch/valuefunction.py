...
class PowerFunctionGrid:
    ...
    def __init__(self,W1,J1,weight=0.01):
        self.num_z, _ , self.num_K = J1.shape
        self.gamma_all = np.zeros( (self.num_z,self.num_K,5) )
        self.rsqr  = np.zeros( (self.num_z,self.num_K))
        self.weight = weight

        # we fit for each (z,x)
        p0 = [0, -1, -1, np.log(0.1)]
        for ix in range(self.num_K):
    ...
    def update(self,W1,J1,lr,nsteps):
        ...
        return(mean_update/(self.num_K*self.num_z))
    ...
    def update_cst(self,W1,J1,lr,nsteps):
        ...
        return(tot_update_chg/(self.num_K*self.num_z))
...
class PowerFunctionGrid2:
    ...
    def __init__(self,W1,J1,vmax, gpow= np.arange(0.0,20.0,1) ,weight=0.01):
        ...
        self.gamma_all = np.zeros( (self.num_z,self.num_K,self.num_g) )
        self.rsqr = np.zeros( (self.num_z,self.num_K))
        self.weight = weight
        self.vmax = vmax
