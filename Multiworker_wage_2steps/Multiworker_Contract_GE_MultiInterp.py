...
class MultiworkerContract:
    ...
    def J_sep(self,Jg=None,Wg=None,Ug=None,Rhog=None,P=None,kappa=None,n0_g = None, sep_g = None,update_eq=1,s=1.0):    
        ...
        #Uploading stuff onto the GPU
        d_rho_grid = cuda.to_device(rho_grid)
        d_N_grid   = cuda.to_device(N_grid)
        d_N_grid1  = cuda.to_device(N_grid1)     
        # Allocate output array on GPU.
        d_rho_star = cuda.device_array(rho_star.shape, dtype=np.float64)

        total_threads = np.prod(rho_star.shape)
        ...
        for ite_num in range(self.p.max_iter):
            ...
            #(n_0+n_1)*rho'_1-EJderiv*eta*(n_0+n_1)-n_0*rho_0-n_1*rho_1
             ...
             #Main foc, in the absence for separations #NOTE THAT WE DO HAVE SEPARATIONS NOW SO THIS FOC IS WRONG!
            ...
            #rho_star_kernel[blocks_per_grid, threads_per_block](d_foc, d_rho_grid,
            #                                              d_N_grid, d_N_grid1, self.p.n_bar1, d_rho_star,self.p.num_z, self.p.num_n, self.p.num_n1, self.p.num_v, self.p.num_q)
            #cuda.synchronize()
            #rho_gpu = d_rho_star.copy_to_host()
            #end_gpu = time()
            #print("GPU time", end_gpu-beg_gpu)
            ...
            #This should be done earlier now??? No, just keeping in mind the previous guess
            if ite_num>=20:
                ...
                for iz in range(self.p.num_z):
                    J_s[iz,...] = RegularGridInterpolator((N_grid, N_grid1, rho_grid, Q_grid), ERho_s[iz, ...], bounds_error=False, fill_value=None) ((n0_star2[iz, ...,ax], n1_s[iz,...], rho_star2[iz, ...,ax], q_s[iz, ...]))
                ...
                if ite_num % 2 == 0:
                    ...
                    sep_star = solve_sep(sep_star,foc_sep,sep_grid,self.p.num_z,self.p.num_v,self.p.num_n,self.p.num_n1,self.p.num_q,0)
                    sep_star = impose_increasing_policy(sep_star) #This may work for juniors... but would it work for seniors?
                    sep_star = 0.4 * sep_star + 0.6 * sep_star0
                else:
                    foc_sep = J_s_deriv - size[...,ax,1] * (re_star[...,ax]+EW_star[...,ax] - EU) * rho_grid[ax,ax,ax,:,ax,ax]
                    foc_sep = impose_increasing_fsep(-foc_sep)
                    sep_star1 = solve_sep(sep_star,foc_sep,sep_grid,self.p.num_z,self.p.num_v,self.p.num_n,self.p.num_n1,self.p.num_q,1)
                    sep_star1 = impose_decreasing_policy(sep_star1)
                    sep_star1 = 0.4 * sep_star1 + 0.6 * sep_star0
            ...
            for iz in range(self.p.num_z):
                for in00 in range(self.p.num_n):
                    ...
                    Rhod0[iz, ..., in00] = Rho_interpolator((n1_star2[iz, ...], rho_star2[iz, ...], q_star2[iz, ...]))
            end=time()
            print("RegularGridInterpolator interpolation", end - beg)
            points = []
            beg=time()
            for iz in range(self.p.num_z):
                points.append( tuple_into_2darray((n1_star[iz, ...], rho_star[iz, ...], q_star[iz, ...])))
            Rhod0_alt = Rhod0_int(Rhod0,ERho,N_grid1,rho_grid,Q_grid,points,self.p.num_z,self.p.num_n)
            end=time()
            print("jitted interpolation", end - beg)
            print("Rhod0 diff",np.mean(np.abs(Rhod0-Rhod0_alt)))
            if ite_num >= 10:
                n0_star[...] = 0
