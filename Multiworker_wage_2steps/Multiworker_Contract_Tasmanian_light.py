...
class MultiworkerContract:
    ...
    def __init__(self, depth, input_param=None, js=None):
        ...
        print(self.gridE[self.p.z_0-1].getPoints().shape)
    ...
    def J_sep(self,Jg=None,Wg=None,Ug=None,Rhog=None,update_eq=0):    
        ...
        for ite_num in range(self.p.max_iter):
            ...
            #Future optimal expectations
            ERho_star_diff = interp_multidim(n0_star,N_grid,np.moveaxis(Rhod0,-1,0))
            #ERho_star = np.zeros_like(ERho_star)
            for iz in range(self.p.num_z):
                ...
                ERho_star[iz,...] = results.reshape(n0_star[iz, ...].shape)
