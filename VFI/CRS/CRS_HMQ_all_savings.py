...
from Simple_model_J_sev_saving import SimpleModel
import numba as nb
...
#@nb.njit(cache=True)
#def utility_1d(log,c):
#    if log:
#        return 1/c
#    else:
#        print("wrong inv utility in optimal savings")
#    return 1/c
@nb.njit(cache=True)
def optimal_savings(c_star,a_star,c_future,c,pu,ass_grid,log):
            ...
            for a in range(ass_grid.shape[0]):
                c_future[a] = interp(a_star[a],ass_grid,c[a,:]) #Predicting future c based on our current guess of the optimal savings
            foc_a = (1-pu[ax,:]) * (1/c_future[ax,:]) - (1/c)
            #Confirming the increasing point: as a' goes up, c goes down, so -u'(c) goes down...
            #as a' goes up, (1-pu) goes up (bruh), c' goes up, so u'(c') goes down??? so should I
            for a in range(ass_grid.shape[0]):
                a_star[a] = interp(0,
                               impose_increasing(-foc_a[a,:]),
                               ass_grid)  
                c_star[a] = interp(a_star[a],ass_grid,c[a,:])  #Could we have done it with c_star from the get-go? mayybe... 
            return c_star,a_star 
class MultiworkerContract:
    ...
    def __init__(self, input_param=None, js=None, log=True,print_choice_simple=False):
        ...
        self.ass_grid = self.simple_model.ass_grid #To make sure the two stay consistent
        self.w_grid = self.simple_model.w_grid
        self.rho_grid = self.simple_model.rho_grid #Note that that I dropped the rho grid very low! Even below the unemp production (is that still okay, or should I be worried about participation constraints?)
    ...
    def J_sep(self,update_eq=1,s=1.0, print_choice = False):    
        ...
        #Severance values
        #a_star = self.simple_model.a_star
        #c_future = np.zeros_like(ass_grid)
        ...
        ite_num = 0
        error_js = 1
        profit = self.fun_prod * self.qual_prod - self.w_grid[ax,:,ax]
        for ite_num in range(self.p.max_iter):
            ...
            #c_star,a_star = optimal_savings(c_star,a_star,c_future,c,pu,ass_grid,self.log)
            ...
            if ite_num>=1:
                ...
                #EJ_s = Rho_s #- rho_star2[...,ax] * W_s
                J_s_deriv = get_fin_diff(grid=sep_reshaped,values=Rho_s,out=J_s_deriv)
                _, re_s, pc_s = self.getWorkerDecisions(W_s)
                #For this one J_s_deriv takes the derivative of Rho_s * (1-sep_reshaped)
                #foc_sep = (J_s_deriv + rho_grid[ax,:,ax,ax] * W_s) * pc_s - (re_s+W_s - EU_star[ax,:,ax,ax]) * rho_grid[ax,:,ax,ax] - sev_star[ax,:,ax,ax]
                #Here J_s deriv is done with Rho_s
                foc_sep = (J_s_deriv * (1-sep_reshaped) - (Rho_s - rho_star2[...,ax] * W_s) ) * pc_s - (re_s+W_s - EU_star[ax,:,ax,ax]) * rho_grid[ax,:,ax,ax] - sev_star[ax,:,ax,ax]
            ...
            rho_star,sep_star = solve_policies(ite_num,rho_star,sep_star,foc_sep,sep_grid, rho_grid, foc, self.p.num_z, self.p.num_v, self.p.num_q)
            ...
            #_,ru_fut,_ = self.getWorkerDecisions(EU_fut, employed=False)
            #U = self.pref.utility(c_star) + self.p.beta * (ru_fut + EU_fut)
            # get unemployed decisions for each severance value
            #pu, ru, _ = self.getWorkerDecisions(EU-self.pref.utility(np.maximum(ass_grid,1.0)), employed=False) #we removing this because.. ok the payoff is ((1-pu)*EU + pu * (vu + ass_grid)), but currently the ass_grid part wouldn't be internalized soooo
            #pu, ru, _ = self.getWorkerDecisions(EU, employed=False)            
            #But wait, the optimization part woould be over pu*(vu-(EU-ass_grid)) #then what is ru here? ru=pu*(vu-(EU-ass_grid)), right? then we need to have the utility component beforehand, too, no?
            #More directly
            #vu = self.js.ve(EU-self.pref.utility(np.maximum(ass_grid,1.0)))
            #pu = self.js.pe(EU-self.pref.utility(np.maximum(ass_grid,1.0)))
            #saving_value = self.pref.utility(np.fmax(c,1e-10)) + self.p.beta * ( (1-pu[ax,:]) * EU[ax,:] + pu[ax,:]*(vu[ax,:]+self.pref.utility(np.maximum(ass_grid[ax,:],1.0)))) #Is this too much? In that, does the unemp value shoot way up because of this?
            #saving_value = self.pref.utility(np.fmax(c,1e-10)) + self.p.beta * ( EU[ax,:] + ru[ax,:])
            #OR, just force search as if zero
            vu = self.js.ve(EU[0])
            pu = self.js.pe(EU[0])
            saving_value = self.pref.utility(np.fmax(c,1e-10)) + self.p.beta * ( (1-pu) * EU[ax,:] + pu*vu)
            ...
            #    rho_grid[ax,:,ax] * W - self.p.beta * sep_star * sev_star[ax,:,ax]   #I've now introduce the cost to layoffs        
            ...
            #J = -impose_increasing_W(-J) #to ensure J is decreasing in v (somehow necessary???) man just how many fucking checks do I need to do???
            ...
            #Jp = Rhop - rho_grid[ax,:,ax]*Wp #THis guy aint increasing in q!!! Why tho?? It should be!
            ...
            if (((ite_num % 10) == 0)):
                ...
                if update_eq:
                    ...
                    # ------ or update search fsunction parameter using relaxation ------
                            ...
                            error_js = self.js.update(Wp[self.p.z_0-1, :, 0], P_xv, type=1, relax=relax)
            ...
            if print_choice:
             if (((ite_num % 1000)  == 0) & (ite_num>10)):
                ...
                plt.plot(W[self.p.z_0-1, :, 0], J [self.p.z_0-1, :, 0], label='1 senior value function')
                plt.plot(W[self.p.z_0-1, :, 0], Jp[self.p.z_0-1, :, 0], label='1 senior value function') 
        ...
        # find rho_u2e
        self.ve = self.js.ve(EU[0]) #REMEMBER TO REMOVE THIS!!!
        Pr_u2e = self.js.pe(EU[0]) # this does not include the inefficiency of search for employed
        ...
        self.save("model_GE_all_saving.pkl")
        return self
    ...
    def append_results_to_pickle(self, J, W, U, Rho, EW_star, sep_star, pickle_file="results_GE_all_saving.pkl"):
        # Step 1: Load the existing data from the pickle file
        try:
            with open(pickle_file, "rb") as file:
                all_results = pickle.load(file)
