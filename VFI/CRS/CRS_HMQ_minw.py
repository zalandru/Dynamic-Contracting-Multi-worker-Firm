...
from Simple_model_J import SimpleModel
import numba as nb
...
@nb.njit(cache=True, parallel=False)
def solve_policies(ite_num, rho_minw_threshold, rho_star, sep_star, foc_sep, sep_grid, beta, u_min_wage, re, W,
    ...
    for iz in prange(num_z):
        for iv in prange(num_v):
            for iq in prange(num_q):
                        #rho_minw_threshold[iz,iv,iq] = interp(0,beta * re[iz,iv,:,iq] - W[iz,iv,iq] + u_min_wage, rho_grid) #like this or the opposite? This thing is increasing in rho and should generally be negative. So the zero is reached when rho is "large"
                        ...
                        if ite_ge_20:
                            sep_star[iz, iv, iq] = interp(0,
                                                                    foc_sep[iz, iv, iq, :],
                                                                    sep_grid)
    return rho_minw_threshold,rho_star, sep_star
...
class MultiworkerContract:
    ...
    """
    def __init__(self, input_param=None, js=None, log=True):
        """
        ...
        self.w_matrix = np.zeros(self.simple_J.shape)
    ...
    def J_sep(self,update_eq=1,s=1.0, print_choice = False):    
        ...
        #Min wage utility
        u_min_wage = self.pref.utility(self.p.min_wage)
        rho_minw_threshold = np.zeros_like(J)
        ...
        for ite_num in range(self.p.max_iter):
            ...
            _, re, pc = self.getWorkerDecisions(EW) #so HERE we want to somehow use this re to check for \beta re <= W - u(self.p.min_wage) #Is this correct check tho? Ahhhh not exactly, but close enough ig. Otherwise, gotta do the same thing as in the layoff foc
            ...
            if ite_num>=1:
                ...
                #Rho_s_f = np.zeros_like(J)
                #W_s_f = np.zeros_like(W)
                #for iz in range(self.p.num_z):
                #    for iq in range(self.p.num_q):
                #        Rho_s_f[iz,:,iq] = np.interp(rho_star[iz,:,iq],self.rho_grid,ERho[iz,:,iq]) #First, we interpolate the v component, always the same
                #        W_s_f[iz,:,iq] = np.interp(rho_star[iz,:,iq],self.rho_grid,EW[iz,:,iq])                

                #for iz in range(self.p.num_z):
                # for iv in range(self.p.num_v):
                #    for iq in range(self.p.num_q):                
                #        Rho_s[iz,iv,iq,:] = np.interp(q_s[iq,:],self.Q_grid,Rho_s_f[iz,iv,:])# Next, interpolate the future quality component. So we get EJ'(rho*,q'(q,sep))
                #        W_s[iz,iv,iq,:] = np.interp(q_s[iq,:],self.Q_grid,W_s_f[iz,iv,:])
                ...
                foc_sep = (J_s_deriv * (1-sep_reshaped) - (Rho_s - rho_star[...,ax] * W_s) ) * pc_s - (re_s+W_s - EU) * rho_grid[ax,:,ax,ax]
                #foc_sep = J_s_deriv * pc_s - (re_s+W_s - EU) * rho_grid[ax,:,ax,ax]
                foc_sep = impose_increasing_fsep(-foc_sep)


            rho_minw_threshold, rho_star,sep = solve_policies(ite_num, rho_minw_threshold, rho_star,sep_star,foc_sep, sep_grid, self.p.beta, u_min_wage, re, W, rho_grid, foc, self.p.num_z, self.p.num_v, self.p.num_q)
            rho_star = impose_increasing_policy(rho_star)
            #rho_star[rho_star > rho_minw_threshold] = rho_minw_threshold[rho_star > rho_minw_threshold] #This may be off since rho_minw_threshold is not a constant
            #Okay... is that it??? Should we also set the wage accordingly????? That part is pretty weird tbh. in the cases where rho_star hits the threshold, we should get min wage.
            #BUT! Don't we get min wage just for the low rho values? and here rho_star increases with rho? yes, but lower rho also tightens the threshold
            if ite_num >= 1:
            ...
            #w = self.pref.inv_utility(W - self.p.beta * (re_star+EW_star)) # AH FUCK! THAT DOESN'T WORK!!! I'd just end up with W=W... so do I abandon W and just work with derivatives of Rho??
            W = self.pref.utility(self.w_matrix) + \
                self.p.beta * (sep_star * EU + (1 - sep_star) * (EW_star + re_star)) #Will this still be correct even when w_grid=min_wage?
        ...
        self.Pr_u2e = Pr_u2e
        self.sev_star = np.zeros_like(self.rho_grid) #To have the severance for the simulator
    ...
    def append_results_to_pickle(self, J, W, U, Rho, EW_star, sep_star, pickle_file="results_GE_minw.pkl"):
        # Step 1: Load the existing data from the pickle file
        try:
            with open(pickle_file, "rb") as file:
                all_results = pickle.load(file)
