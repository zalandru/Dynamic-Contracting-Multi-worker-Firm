...
class Simulator:
    ...
    def computeMoments(self):
        ...
        sdata_y_f = (sdata_y_f.join(hdata.ypw, on="h")
               .pipe(create_year_lag_by, ['w','ypw','s'], 1, by=('i','fid'))
               .assign(dw=lambda d: d.w - d.w_l1,
                       dypw=lambda d: d.ypw - d.ypw_l1,
                       dten=lambda d: d.s - d.s_l1)
        ...
        #            .assign(dw=lambda d: d.w - d.w_l1,
        #                  dypw=lambda d: d.ypw - d.ypw_l1))
        #1. Proportion of new hires out of all the employed workers
        #moms['pr_new_hire_s'] = sdata.eval('s==0 & e==1').sum() / sdata.eval('e==1').sum()
        moms['pr_new_hire'] = sdata.eval('tenure==0 & e==1').sum() / sdata.eval('e==1').sum() #this is kinda fake? in that new hires are basically evaluated 4 times here???
        moms['pr_new_hire_y_f'] = sdata_y_f.eval('s<=1 & e==1').sum() / sdata_y_f.eval('e==1').sum() #this is more precise, no?
...
def debug(load):
    ...
    if load:
        ...
        from VFI.CRS.CRS_HMQ import MultiworkerContract
