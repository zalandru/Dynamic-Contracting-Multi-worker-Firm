...
class SimpleModel:
    ...
    def __init__(self, input_param=None, js=None):
        ...
        self.sev_grid = np.linspace(0, self.fun_prod_onedim[self.p.z_0-1]-self.unemp_bf, 100 )
    ...
    def J_sep(self,Jg=None,Wg=None,Ug=None,Rhog=None,P=None,kappa=None,n0_g = None, sep_g = None,update_eq=1,s=1.0,layoff_iter=1,print_choice=False):    
        ...
        if Ug is None:
            U = self.pref.utility(self.unemp_bf + sev_grid) / (1 - self.p.beta)
        else:
            U = np.copy(Ug)
        if sep_g is None:
        ...
        for ite_num in range(self.p.max_iter):
            ...
            usearch = np.argmax( self.pref.utility(self.unemp_bf+sev_grid[:,ax]) + self.p.beta * ite_prob_vx[ax,:] *
                     (W[ax, self.p.z_0 - 1, :, 0] - EU[:,ax]) + self.p.beta * EU[:,ax], axis=1)
            pu = ite_prob_vx[usearch]
            pu_deriv[1:-1] = (pu[2:] - pu[:-2]) / (sev_grid[2:] - sev_grid[:-2])
            pu_deriv[0]    = (pu[1]  - pu[0])  / (sev_grid[1]  - sev_grid[0])
            pu_deriv[-1]   = (pu[-1] - pu[-2]) / (sev_grid[-1] - sev_grid[-2])
            #print("pu deriv range", pu_deriv.min(),pu_deriv.max())
            assert np.all(pu_deriv <= 0)
            ...
            mask = sev_check[0,:] > 0
            sev_foc = (1 - self.p.beta * self.sev_grid * pu_deriv / (1-self.p.beta * (1-pu))) / self.pref.utility_1d(self.unemp_bf+self.sev_grid)
            sev_star = np.interp(rho_grid,
                            impose_increasing(sev_foc) ,
                            self.sev_grid)#Question: is this still the same even now? I don't think OJS affects it tbf
            EU_star = np.interp(sev_star,self.sev_grid,EU)   
            pu_star = np.interp(sev_star,self.sev_grid,pu)
            assert np.all(sev_star[mask]>0) #okay this assetion is not failed eh... so this isn't just a coding issue i guess...
            ...
            if ite_num>=layoff_iter:
                ...
                foc_sep = J_s_deriv * (1-sep_reshaped) - (Rho_s - rho_grid[ax,:,ax,ax] * W_s) - (W_s - EU_star[ax,:,ax,ax]) * rho_grid[ax,:,ax,ax] - sev_star[ax,:,ax,ax] / ( 1 - self.p.beta*(1-pu_star[ax,:,ax,ax]))#Basically, here I take the derivative of (1-sep) * pc_star * EJ wrt sep. Since pc_star is a "coonstant" here, I look at how (1-sep) * EJ changes
            ...
            Rho = self.fun_prod * self.qual_prod - self.w_grid[ax,:,ax] + rho_grid[ax,:,ax] * W - self.p.beta * sep_star * sev_star[ax,:,ax] / ( 1 - self.p.beta*(1-pu_star[ax,:,ax])) + self.p.beta * (1-sep_star) * (ERho_star - rho_grid[ax,:,ax] * EW_star)
            ...
            U = np.max( self.pref.utility(self.unemp_bf+sev_grid[:,ax]) + self.p.beta * ite_prob_vx[ax,:] *
                               (W[ax, self.p.z_0 - 1, :, 0] - EU[:,ax]) + self.p.beta * EU[:,ax], axis=1)
