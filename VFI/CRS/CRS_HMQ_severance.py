...
class MultiworkerContract:
    ...
    def J_sep(self,update_eq=1,s=1.0, print_choice = False):    
        ...
        for ite_num in range(self.p.max_iter):
            ...
            # get unemployed decisions at EU + epsilon
            #pu_d, _, _ = self.getWorkerDecisions(EU + self.deriv_eps, employed=False) 
            #Wait, we need an actual derivative here, no? wrt sev
            #pu_deriv[0] = (pu[1] - pu[0]) / (self.sev_grid[1] - self.sev_grid[0])
            #pu_deriv[:-1] = (pu[1:] - pu[:-1]) / (self.sev_grid[1:] - self.sev_grid[:-1])
            #pu_deriv[-1] = (pu[-1] - pu[-2]) / (self.sev_grid[-1] - self.sev_grid[-2])
            ...
            if ite_num>=1:
                ...
                foc_sep = (J_s_deriv * (1-sep_reshaped) - (Rho_s - rho_star[...,ax] * W_s) ) * pc_s - (re_s+W_s - EU_star[ax,:,ax,ax]) * rho_grid[ax,:,ax,ax]
    ...
    def append_results_to_pickle(self, J, W, U, Rho, EW_star, sep_star, pickle_file="results_GE_sev.pkl"):
        # Step 1: Load the existing data from the pickle file
        try:
            with open(pickle_file, "rb") as file:
                all_results = pickle.load(file)
