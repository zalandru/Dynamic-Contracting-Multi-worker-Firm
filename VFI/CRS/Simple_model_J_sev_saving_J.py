...
class SimpleModel:
    ...
    def J_sep(self,Jg=None,Wg=None,Ug=None,Rhog=None,P=None,kappa=None,n0_g = None, sep_g = None,update_eq=1,s=1.0,layoff_iter=1,print_choice=False):    
        ...
        for ite_num in range(self.p.max_iter):
            ...
            #Getting optimal unemployed search: 1st dimension is FUTURE savings, second dimension is where the worker chooses to search
            ...
            for a in range(ass_grid.shape[0]):
                c_future[a] = np.interp(a_star[a],ass_grid,c[a,:])
            foc_a = (1-pu[ax,:]) * self.pref.utility_1d(c_future[ax,:]) - self.pref.utility_1d(c)
            ...
            if ite_num>=layoff_iter:
                ...
                EJ_s = np.zeros_like(J_s_deriv)
                for iz in range(self.p.num_z):
                 for iv in range(self.p.num_v):
                    for iq in range(self.p.num_q):                
                        W_s[iz,iv,iq,:] = np.interp(q_s[iq,:],self.Q_grid,EW[iz,iv,:])
                        EJ_s[iz,iv,iq,:] = np.interp(q_s[iq,:],self.Q_grid,EJ[iz,iv,:])
                #EJ_s = Rho_s - rho_grid[ax,:,ax,ax] * W_s
                Rho_s = EJ_s + rho_grid[ax,:,ax,ax] * W_s
                #J_s_deriv[...,0] = (EJ_s[...,1] * (1-sep_grid[1]) - EJ_s[...,0] * (1-sep_grid[0])) / (sep_grid[1] - sep_grid[0])
                #J_s_deriv[...,-1] = (EJ_s[...,-1] * (1-sep_grid[-1]) - EJ_s[...,-2] * (1-sep_grid[-2])) / (sep_grid[-1] - sep_grid[-2]) 
                #J_s_deriv[..., 1:-1]    = (EJ_s[...,2:] * (1-sep_reshaped[...,2:]) - EJ_s[...,:-2] * (1-sep_reshaped[...,:-2])) / (sep_reshaped[...,2:] - sep_reshaped[...,:-2]) 
                ...
                #foc_sep = J_s_deriv - (W_s - EU_star[ax,:,ax,ax]) * rho_grid[ax,:,ax,ax] - sev_star[ax,:,ax,ax]
                #foc_sep = J_s_deriv * (1-sep_reshaped) - J_s - (W_s - EU_star[ax,:,ax,ax]) * rho_grid[ax,:,ax,ax] - sev_star[ax,:,ax,ax]
            ...
            for iz in range(self.p.num_z):
                for iv in range(self.p.num_v):
                    EJ_star[iz,iv,:] = np.interp(q_star[iz,iv,:],self.Q_grid,EJ[iz,iv,:])
            ...
            J = self.fun_prod * self.qual_prod - self.w_grid[ax,:,ax] - self.p.beta * sep_star * sev_star[ax,:,ax] + self.p.beta * (1-sep_star) * EJ_star
