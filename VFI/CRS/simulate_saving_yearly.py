...
def create_t_lag(df,colnames,lag):
    """ the table should be index by i,year
    """
    # prepare names
    if lag>0:
    ...
    # create lags
    dlag = df.reset_index() \
             .assign(t=lambda d: d['t'] + lag) \
             .rename(columns=rename)[['i','t'] + values] \
             .set_index(['i','t'])
...
class Simulator:
    ...
    def simulate_val(self,ni=int(1e4),nt=100,burn=20,nl=100,redraw_zhist=True,ignore=[]):
        ...
        for t in range(nt):
            ...
            for ix in range(p.num_x): 
                ...
                if len(model.Pr_u2e.shape)==0:
                    Pr_u2e = model.Pr_u2e
                else:
                    Pr_u2e = np.interp(Sev[Ix],model.ass_grid,model.Pr_u2e)
                ...
                if len(model.Pr_u2e.shape)==0:
                    R[Ix_u2e]  = model.rho_u2e
                else:
                    R[Ix_u2e]  = np.interp(Sev[Ix_u2e],model.ass_grid,model.rho_u2e)                      # find the firm and the initial rho
                ...
                S[Ix_u2e]  = 1
                Sev[Ix_u2e] = 0 #again, their assets become zero
                ...
                R[Ix_u2u]  = 0
                S[Ix_u2u]  = 0
                Sev[Ix_u2u] = np.interp(Sev[Ix_u2u],model.ass_grid,model.a_star) #Updating the remaining assets of the unemployed workers
                V[Ix_u2u] = np.interp(Sev[Ix_u2u],model.ass_grid,model.Vf_U)
            ...
            for iz in range(p.num_z):
                    ...
                    H[Ix_e2u]   = -1
                    S[Ix_e2u]   = 1
                    Sev[Ix_e2u] = np.interp(R[Ix_e2u],model.rho_grid,model.sev_star) #we get the actual severance pay that the firm would've promised
                    R[Ix_e2u]   = 0
                    V[Ix_e2u] = np.interp(Sev[Ix_e2u],model.ass_grid,model.Vf_U)
                    ...
                    D[Ixz_j2j]   = Event.j2j
                    W[Ixz_j2j]   = np.log(np.interp(R[Ixz_j2j],model.rho_grid,model.w_grid)) #Wait, this would not include min wage
                    ...
                    D[Ixz_ee]   = Event.ee
                    W[Ixz_ee]   = np.log(np.interp(R[Ixz_ee],model.rho_grid,model.w_grid))
        ...
        df_all['year'] = df_all['t'] #(df_all['t'] - (df_all['t'] % 4))//4

        #construct yearly tenure
        df_all['tenure'] = df_all['s'] #np.floor(df_all['s'] / 4)
    ...
    def computeMoments(self):
        ...
        emp = sdata.query('h>0').copy()
        emp['w'] = np.exp(emp['w_net'])
        moms['workers_on_min_w'] = emp.eval('w<=1.05 * @self.p.min_wage').sum()/len(emp)
        # ------  transition rates   -------
        ...
        #hdata['ypw'] = np.log(hdata.f_year/hdata.c_year)
        #hdata['lsize'] = np.log(hdata.c_year/4) # log number of worker in the year
        hdata['c_year'] = hdata.i
        hdata['ypw'] = np.log(hdata.f / hdata.i)
        hdata['lsize'] = np.log(hdata.i)

        # create year on year growth at the firm level
        hdata['le2u'] = np.log(hdata['c_e2u'] / hdata['c_year'])
        hdata['lj2j'] = np.log(hdata['c_j2j'] / hdata['c_year'])
        hdata['lsep'] = np.log((hdata['c_j2j'] + hdata['c_e2u']) / hdata['c_year'])
        hdata = hdata.drop(columns='i')

        # add measurement error to ypw
        hdata_sep = (hdata.assign(ypwe=lambda d: d.ypw + self.p.prod_err_y * np.random.normal(size=len(d.ypw)))
                          .pipe(create_lag, 'h', ['ypw', 'ypwe', 'le2u', 'lj2j', 'lsep'], 1)
                          .assign(dlypw=lambda d: d.ypw - d.ypw_l1,
                                  dlypwe=lambda d: d.ypwe - d.ypwe_l1,
                                  dle2u=lambda d: d.le2u - d.le2u_l1,
                                  dlsep=lambda d: d.lsep - d.lsep_l1,
                                  dlj2j=lambda d: d.lj2j - d.lj2j_l1)[['dlypw', 'dlypwe', 'dle2u', 'dlj2j', 'dlsep', 'c_year']])
        ...
        sdata_y = sdata.groupby(['i', 'year']).agg({'w_exp': 'sum', 'h': 'min', 's': 'min', 's2': 'max', 'e': 'min', 'es': 'sum', 'tenure': 'min'}) #wait, do I want the sum of the wages here? I guess if the workers are here the whole time, it's no biggie?
        sdata_y = sdata_y.pipe(create_year_lag, ['e', 's'], -1).pipe(create_year_lag, ['e', 'es'], 1)
        # make sure we stay in the same spell, and make sure it is employment
        #sdata_y = sdata_y.query('h>=0').query('s+3==s2')
        sdata_y['w'] = np.log(sdata_y['w_exp'])
        ...
        employed = sdata_y_f.copy().query(' tenure<= 10') #tenure starts at zero, so 10 years!!!
        moms['avg_w_growth_10'] = employed.groupby('tenure')['dw'].mean().sum()
        employed = sdata_y.copy().query(' tenure<= 90') #tenure starts at zero, so 10 years!!!11
        moms['avg_w_growth_10_y'] = employed.groupby('tenure')['dw'].mean().sum()  #Recheck for the case of sdata_y
        #Souchier-type w growth moment: regress wage on tenure
        moms['avg_ten_prof_10'] = feols('w ~ C(tenure)', data=sdata_y).coef()['C(tenure)[T.10]']
        ...
        hdata['layoff_rate'] = hdata['c_e2u'] / (hdata['c_year'] )
    ...
    def model_evaluation(self):
        ...
        :return:
        """
        sdata_y_f = self.sdata_y_f.query('tenure<=10')
        #OR TRY WITH SDATA_Y??? I dunno why I'd need to somehow it's not working sooooo
        sdata_y = self.sdata_y.query('tenure<=10')
        #hdata = self.hdata
        moms_untarg = {}
    ...
    def _tenure_stats_from_sdata(self, sdata: pd.DataFrame, use_promised: bool = True):
        ...
        g = (emp.query('tenure<=9') #to only ever have first 10 years of tenure
         .groupby('tenure')
         .agg(w=('w_exp', 'mean'),
              v=('v', 'mean'),
              sev=('sev_use', 'mean')))
    ...
    def _mandated_severance_curve(self, tenure_index, moms_mean):
        ...
        mandated = (1/12) * avg_w * tenure_vals #NOTE!!!! THIS IS TOTAL SEVERANCE PAY!!!! 
