...
class MultiworkerContract:
    ...
    def J_sep(self,Jg=None,Wg=None,Ug=None,Rhog=None,P=None,kappa=None,n0_g = None, sep_g = None,update_eq=1,s=1.0,layoff_iter=20):    
        """
        ...
        for ite_num in range(self.p.max_iter):
            ...
            if ite_num>=layoff_iter:
                ...
                foc_sep = J_s_deriv * pc_star[...,ax] - (re_star[...,ax]+EW_star[...,ax] - EU) * rho_grid[ax,:,ax,ax] #Basically, here I take the derivative of (1-sep) * pc_star * EJ wrt sep. Since pc_star is a "coonstant" here, I look at how (1-sep) * EJ changes
            ...
            #how does prod-ty develop with layoffs right now (1-sep) * y * ( prod_q + q/(1-sep)*(1- prod_q) ) =  y * (prod_q * (1-sep) + q * (1-prod_q)) #so yep, layoffs just shed bad matches, and those are more productive in high periods. so you want to fire less!
                ...
                sep_star = - impose_increasing_policy(-sep) #+ 0.6 * sep_star0
            ...
            J= self.fun_prod * self.qual_prod - self.w_grid[ax,:,ax] + self.p.beta *  pc_star * (1-sep_star) * EJ_star #Note it's not EJ_star here anymore. To make the function faster
            ...
            W = .4 * W + .6 * W2 #we're completely ignoring the 0th step
            ...
            # Updating J1 representation
            if s is None:
                error_j1p_chg, rsq_j1p = J1p.update_cst_ls(W, J)
                Wp = W
                Jp = J1p.eval_at_W1(W)
                error_j1i = array_exp_dist(J,Jp, 100)
            else:
                for iz, iq in indices_no_v:
                    Wp[iz,:,iq] = splev(rho_grid, splrep(rho_grid,W[iz,:,iq],s=s))
                Wp = impose_increasing_W(Wp)
                J = .2 * J + .8 * J2
                for iz, iq in indices_no_v:    
                    Jp[iz,:,iq] = splev(Wp[iz,:,iq], splrep(Wp[iz,:,iq],J[iz,:,iq],s=s)) #Should I go back to the BL-style smoothing? This would have value at least due to fitting the same kind of polynomial every time
                error_j1i = array_dist(J,J2) #np.power(J - J2, 2).mean() / np.power(J2, 2).mean()  
        ...
        self.append_results_to_pickle(J, W, U, P, kappa, EW_star, sep_star)
    ...
    def GE(self,EJ,W,kappa_old=None,J=None,n0_star=None):
        ...
        return kappa, P
    def append_results_to_pickle(self, J, W, U, P, kappa, EW_star, sep_star, pickle_file="results_GE.pkl"):
        # Step 1: Load the existing data from the pickle file
        try:
            with open(pickle_file, "rb") as file:
                all_results = pickle.load(file)
        ...
        new_results = self.save_results_for_p(J, W, U, P, kappa, EW_star, sep_star)
    ...
    def save_results_for_p(self, J, W, U, P, kappa, EW_star, sep_star):
