...
from Simple_model_J_sev_saving_J import SimpleModel
import numba as nb
...
class MultiworkerContract:
    ...
    def J_sep(self,update_eq=1,s=1.0, print_choice = False):    
        ...
        for ite_num in range(self.p.max_iter):
            ...
            pu, _, _ = self.getWorkerDecisions(EU, employed=False)
            ...
            foc = rho_grid[ax, ax, :, ax] - rho_grid[ax, :, ax, ax] - EJpi * log_diff / self.deriv_eps #keep in mind that log_diff here is NOT accounting for future quality
            ...
            if ite_num>=1:
                ...
                J_s,W_s = Values_int(Rho_s,W_s,EJ,EW,rho_grid,Q_grid,points,self.p.num_z)

                Rho_s = J_s + rho_star2[...,ax] * W_s
            ...
            assert np.all(sep_star[1:,...]-sep_star[:-1,...]<=0)
            ...
            EJ_star, EW_star =  Values_int(EJ_star,EW_star,EJ,EW,rho_grid,Q_grid,points,self.p.num_z)
            ...
            #Jp = J
            J = self.fun_prod * self.qual_prod - self.w_grid[ax,:,ax] + self.p.beta * pc_star * (1-sep_star) * EJ_star - \
            ...
            for iz, iq in indices_no_v:
                ...
                Jp[iz,:,iq] = splev(W[iz,:,iq], splrep(W[iz,:,iq],J[iz,:,iq],s=s)) #Should I go back to the BL-style smoothing? This would have value at least due to fitting the same kind of polynomial every time
        ...
        #self.append_results_to_pickle(J, W, U,EW_star, sep_star)
