...
class MultiworkerContract:
    ...
    def J(self,Jg=None,Wg=None,update_eq=0):    
        ...
        for ite_num in range(self.p.max_iter):
            ...
            #EJpi,EW1i = EJs(EJ1_star, EW1_star, EJpi, EW1i, rho_star, rho_grid, self.p.num_z, self.p.num_n, self.p.num_v, self.p.num_q)
    ...
    def J_sep_dir(self,Jg=None,Wg=None,update_eq=0):    
        ...
        for ite_num in range(self.p.max_iter):
            ...
            rho_star = optimized_loop(
                 pc, rho_grid, self.N_grid1, foc, rho_star, self.p.num_z, self.p.num_n, self.p.n_bar, self.p.num_q)
            #Interpolate the expectations onto rho_star
            EJRho,EWRho = EJs(EJ1_star, EW1_star, EJpi, EW1i, rho_star, rho_grid, self.p.num_z, self.p.num_n, self.p.num_v, self.p.num_q)	
            if ite_num>1:
