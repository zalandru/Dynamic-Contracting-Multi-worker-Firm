...
#NOTE: There are very slight changes here still. Mainly the fact that I don't use RegularGridInterpolator anymore, but rather a custom one.
# Also, the bool_index_combine function isn't used because it was giving an error. Otherwise this is the old code (also new moments)
...
class Simulator:
    ...
    def computeMoments(self):
        ...
        hdata = (sdata.set_index(['i', 't'])
                      ...
                      .assign(e2u=lambda d: d.d_f1 == Event.e2u,
                              ...
                              c_j2j=lambda d: d.d_f1 == Event.j2j)
                      .groupby(['f'])
                      .agg( {'prod': 'max', 'i': "count", 'c_e2u': 'sum', 'c_j2j': 'sum'})) #Also we don't want to sum prod! It is already at the firm level so don't sum it up.
        ...
        assert hdata['i'] == hdata['n'] #Also a good check to see if I understand the data correctly
        ...
        hdata['ypw'] = np.log(hdata.prod/hdata.i)
        hdata['dypw'] = hdata.ypw.log() - hdata.pipe(create_lag, 'f', ['ypw'], 1)['ypw_l1'].log() #I want the growth in logs, right? CHECK IN THE DATA. Souchier does it in logs.
        hdata['id_shock_diff'] = hdata['dypw']
        hdata['id_shock_sum'] = hdata.pipe(create_lag, 'f', ['ypw'], -1)['ypw_f1'].log() - hdata.pipe(create_lag, 'f', ['ypw'], 2)['ypw_l2'].log() #Cumulative log sum over the 3 years, 1 forward and 2 back
        hdata['id_shock_sum_lag'] = hdata['ypw'].log() - hdata.pipe(create_lag, 'f', ['ypw'], 3)['ypw_l3'].log()
        ...
        #And to confirm
        assert hdata['id_shock_sum_lag'] == hdata.pipe(create_lag, 'f', ['id_shock_sum'], 1)['id_shock_sum_l1']
        #hdata['lsize'] = np.log(hdata.c_year/4) # log number of worker in the year
        ...
        # add measurement error to ypw
        hdata_sep = (hdata.assign(ypwe=lambda d: d.ypw + self.p.prod_err_y * np.random.normal(size=len(d.ypw)))
                          .pipe(create_lag, 'f', ['ypw', 'ypwe', 'le2u', 'lj2j', 'lsep'], 4)
                          .assign(dlypw=lambda d: d.ypw - d.ypw_l4,
                                  dlypwe=lambda d: d.ypwe - d.ypwe_l4,
                                  dle2u=lambda d: d.le2u - d.le2u_l4,
                                  dlsep=lambda d: d.lsep - d.lsep_l4,
                                  dlj2j=lambda d: d.lj2j - d.lj2j_l4)[['dlypw', 'dlypwe', 'dle2u', 'dlj2j', 'dlsep', 'i']])

        # covaraince between change in log separation and log value added per worker
        moms['cov_dydsep'] = hdata_sep.cov()['dlypw']['dlsep'] #Andrei: I could do the same, right? Except for using e2u instead of all separations

        # moments of the process of value added a the firm level
        cov = hdata_sep.pipe(create_lag, 'h', ['dlypwe'], 4)[['dlypwe', 'dlypwe_l4']].cov() #Andrei: I can work with dlypw instead of dlypwe if I don't want to use the measurement error
        ...
        sdata['w_growth_rate'] = sdata.query('e==1')['w'].log() - sdata.query('e==1').pipe(create_year_lag, ['w'], 1).log() #No this is kinda trash, it just a single growth rate 7 years back
        ...
        moms['avg_w_growth_7'] = sdata.query('e==1').groupby('s')['w_growth_rate'].mean().sum()
        ...
        #First denote opening firms, that is, firms that for the first time (not the second or third) have a non-zero c_year
        hdata['opening_firm'] = hdata.query('i>0').groupby('f').min()['i'] == hdata['i']
        moms['ratio_jobs_opening'] = hdata.query('opening_firm').eval('s==1 & e==1').sum() / hdata.eval('s==1 & e==1').sum()
        ...
        hdata_emp = hdata['e'==1].copy() #Keep only employed workers. But wait, where does even e2u reside? Doesn't it reside only with unemployed workers? Yep, so the issue that I have is that I don't have e2u for employed workers, like I do in the data
        #Now we regress ypw on e2u_emp
        model_ypw = feols('ypw ~ e2u_emp', data=hdata_emp)
        moms['ypw_layoffs'] = model_ypw.coef['e2u_emp']
        model_prod = feols('prod ~ e2u_emp', data=hdata_emp)
    ...
    def computeMoments_old_year_agg(self):
        """
        Computes the simulated moments using the simulated data
        :return:
        """
        ...
        moms['mean_dw_j2j_2'] = (sdata_y
                                    .pipe(create_year_lag, ['w', 'f', 'e'], 2)
                                    .query('e_l2 == 1').query('f_l2 != f') #Andrei: was 'h_l2 + 8 != h'? Why +8? because of some history notation stuff?
        ...
        # In my data, I took the average wage growth for each tenure group, and then summed/multiplied them all.
        # So here, I could take the average wage growth for each s[e==1], and then combine them all. Do I want to have low wages then?
        sdata_y.query('e==1 & s==1')['w'].log() - sdata_y.query('e==1 & s==1').pipe(create_year_lag, ['w'], 7).log() #This is the wage growth over 7 years
        sdata_y['w_growth_rate'] = sdata_y.query('e==1')['w'].log() - sdata_y.query('e==1').pipe(create_year_lag, ['w'], 7).log()
        #Now I wanna take the average wage growth for each tenure group then sum them up
        moms['avg_w_growth_7'] = sdata_y.query('e==1').groupby('s')['w_growth_rate'].mean().sum()
        #So what I want is the following: get the growth rate
        #3. Productivity moments
        #a) s.d. of firm productivity growth (take sd of dypw?) yep, this is exactly what we do in the data
        moms['sd_dypw'] = sdata_y['dypw'].std()
        #b) annual persistence of firm productivity (take autocovariance of ypw?)
        moms['autocov_dypw'] = hdata.pipe(create_year_lag, ['ypw'], 1).pipe(create_year_lag, ['ypw'], 2)[['ypw', 'ypw_l1', 'ypw_l2']].cov()['ypw']['ypw_l1'] #This is kinda weird? This was suggested code, but I don't get it. How does it work? Detailed answer: it takes the lagged ypw, and then computes the covariance between the current and lagged ypw. So it is like a persistence measure, but not really an autocorrelation, because it is not normalized by variance
        #4. Firm dynamics moments
        #a) average firm size (mean of c_year?)
        moms['avg_firm_size'] = hdata['c_year_mean'].mean() #Average number of workers in a firm in a year
        #b) ratio of jobs created by opening firms (need to denote opening firms, then sum up c_u2e_year (doesn't exist yet) for them divide by c_u2e_year for everyone)
        #Wait, do I want u2e? I don't think that's what I did in the data, did I? It may have been jut a ratio of new==1 between opening firms and all firms
        #First denote opening firms, that is, firms that for the first time (not the second or third) have a non-zero c_year
        hdata['opening_firm'] = hdata.query('c_year>0').groupby('f').min()['c_year'] == hdata['c_year']
        moms['ratio_jobs_opening'] = hdata.query('opening_firm').eval('s==1 & e==1').sum() / hdata.eval('s==1 & e==1').sum()
        #c?) proportion of jobs created by firms of certain size (>10/>100 etc. do I need this? if I fix the DRS factor, then not)
        #5. HMQ moments
        #a) Most basic approach: passthrough of productivity wrt layoffs. That's just one moment though. Also this is quite close to one of my untargeted regressions, so maybe I shouldn't use it
        ...
        # Do I do the real labor share here? Or just the prod and ypw? Because in the data it was also kinda meh
        # First, do the fake one, just working with prod and ypw. So need to regress prod and ypw on layoffs, and then take the coefficients
        # Also though, do I regress on EU, or on EU_rate?
        #d) Fabrice suggestion: distribution of layoffs across firm productivity. What would the firm productivity be? I guess the firm output per worker, so ypw?
        # So in the data, I could take terciles of productivity, and then compute the share of layoffs in each tercile. And then same here. Then, if I do terciles, can get rid of the aggregate moment
        #So here, first get the terciles of ypw 
        hdata['ypw_tercile'] = pd.qcut(hdata['ypw'], 3, labels=False)
    ...
    def model_evaluation(self):
        ...
        #Get log wage growth
        self.hdata['w_growth'] = self.hdata['w'].log() - self.hdata.pipe(create_lag_i, 't', ['w'], 1)['w_l1'].log()
        ...
        model = feols('w_growth ~ i(tenure, id_shock_sum)', data=self.hdata)
        moms_untarg = model.coef
