...
def optimized_loop_sep(rho_grid, N_grid1, foc, rho_star, num_z, num_n, n_bar, num_v):
    ...
    return rho_star
@jit(nopython=True, cache=True)
def optimized_loop_tilde(pc, rho_grid, N_grid1, foc, rho_star, num_z, num_n, n_bar, num_v):
    for iz in range(num_z):
...
def n1(pc, rho_grid, rho_star, sep_star, N_grid, N_grid1, num_z, num_n, num_v):
    ...
    return n1
@jit(nopython=True, cache=True)
def n1_tilde(n1,pc,rho_grid,rho_star, sep_star, N_grid, N_grid1, num_z, num_n, num_v):
    for iz in range(num_z):
     for in0 in range(num_n):
      for in1 in range(num_n):         
        for iv in range(num_v):
            n1[iz, in0, in1, :] = (N_grid[in0]*(1-sep_star[iz,in0,in1,:])+N_grid1[in1])*np.interp(rho_star[iz, in0, in1, iv], rho_grid, pc[iz,in0,in1,:,iv])
    return n1
#Given rho_star and n1_star, calculate the future derivative
@jit(nopython=True, cache=True)
def EJderivative(EJpi,floorn1,ceiln1,Ederiv,rho_grid, N_grid1, rho_star,num_z, num_n, n_bar, num_v):
            for iz in range(num_z):
                for in0 in range(num_n):
                    for in1 in range(num_n):
                        for iv in range(num_v):
...
def EJderivative3(EJpi,EW1i, floorn1,ceiln1,Ederiv,rho_grid, N_grid1, rho_star,num_z, num_n, n_bar, num_v):
            ...
            return Ederiv
@jit(nopython=True, cache=True)
def EWderivative(EW1i,floorn1,ceiln1,Ederiv,rho_grid, N_grid1,rho_star,num_z, num_n, n_bar, num_v):
            for iz in range(num_z):
                for in0 in range(num_n):
                    for in1 in range(num_n):
                        for iv in range(num_v):
...
def EW_tild(n1_star,EW1i,N_grid1,num_z,num_n,num_v):
      ...
      for in1 in range(num_n):
       for iv in range(num_v):
        for iv1 in range(num_v):
            EW_tild[iz,in0,in1,iv1,iv] = np.interp(n1_star[iz,in0,in1,iv],N_grid1,EW1i[iz,0,:,iv1])
    return EW_tild
class MultiworkerContract:
    ...
    def J(self,Jg=None,Wg=None,update_eq=0):    
        ...
        Rho = Ji + self.N_grid1[self.grid[2]]*rho_grid[ax,ax,ax,:]*W1i[...,1]        
        ...
        for ite_num in range(self.p.max_iter):
            ...
            if ite_num>100000000:
                EW1_tild = EW_tild(n1_star,EW1i,self.N_grid1,self.p.num_z,self.p.num_n,self.p.num_v)
            ...
            Rhoderiv[:, :, 0, :] = (Rho[:, :, 1, :] - Rho[:, :, 0, :]) / (self.N_grid1[1] - self.N_grid1[0])
            Wderiv[:, :, 0, :]     = (W1i[:, :, 1, :, 1] - W1i[:, :, 0, :, 1]) / (self.N_grid1[1] - self.N_grid1[0])
            # Last boundary condition: backward difference
            Rhoderiv[:, :, -1, :] = Rho[:, :, -1, :] - Rho[:, :, -2, :]/ (self.N_grid1[-1] - self.N_grid1[-2])
            Wderiv[:, :, -1, :]     = W1i[:, :, -1, :, 1] - W1i[:, :, -2, :, 1]/ (self.N_grid1[-1] - self.N_grid1[-2])
            # Central differences: average of forward and backward differences
            Rhoderiv[:, :, 1:-1, :] = (Rho[:, :, 2:, :] - Rho[:, :, :-2, :]) / (self.N_grid1[ax, ax, 2:, ax] - self.N_grid1[ax, ax, :-2, ax])
            Wderiv[:, :, 1:-1, :]     = (W1i[:, :, 2:, :, 1] - W1i[:, :, :-2, :, 1]) / (self.N_grid1[ax, ax, 2:, ax] - self.N_grid1[ax, ax, :-2, ax])


            Jderiv = Rhoderiv-rho_grid[ax,ax,ax,:]*W1i[...,1]

            #Jderiv = Rhoderiv+self.N_grid1[self.grid[2]]*rho_grid[ax,ax,ax,:]*Wderiv #accounting for the fact that size change also impacts W
            ...
            if ite_num > 1:
                for iz in range(self.p.num_z):
                    for in00 in range(self.p.num_n):
                        J_interpolator = RegularGridInterpolator((self.N_grid1, rho_grid), ERho[iz, in00, :, :], bounds_error=False, fill_value=None)
                ...
                #Ihire = ((Jd0[...,1]-Jd0[...,0]+rho_star*n1_star*(Wd0[...,1]-Wd0[...,0])) > self.p.hire_c / self.p.beta) & (self.N_grid[self.grid[1]]+self.N_grid[self.grid[2]] < self.p.n_bar - 1)
            ...
            if ite_num<=100000000:            
                ...
                n1_star = n1_tilde(n1_star,pc,rho_grid,rho_star, sep_star, self.N_grid, self.N_grid1, self.p.num_z, self.p.num_n, self.p.num_v)


            EW1i_interpolators = [RegularGridInterpolator((self.N_grid1, rho_grid), EW1i[iz, 0, :, :], bounds_error=False, fill_value=None) for iz in range(self.p.num_z)]
            EJpi_interpolators = [RegularGridInterpolator((self.N_grid1, rho_grid), EJpi[iz, 0, :, :], bounds_error=False, fill_value=None) for iz in range(self.p.num_z)]
            ...
            EJderiv0 = EJderivative(EJpi, floorn1,ceiln1,EJderiv,rho_grid, self.N_grid1, rho_star,self.p.num_z, self.p.num_n, self.p.n_bar, self.p.num_v)
            EWderiv = EWderivative(EW1i,floorn1,ceiln1,EWderiv,rho_grid, self.N_grid1, rho_star,self.p.num_z, self.p.num_n, self.p.n_bar, self.p.num_v)
            ...
            #Ji[:,0,0,:] = self.fun_prod[:,0,0,:]*self.prod[:,0,0,:] - sum_wage[:,0,0,:] +\
            #     self.p.beta * pc_star[:,0,0,:] * EJ1_star[:,0,0,:] #There are no juniors here so
            Ji = .2 * Ji + .8 * Ji2
            ...
            #The Dual Value Function Rho
            Rho = Ji + self.N_grid1[self.grid[2]]*rho_grid[ax,ax,ax,:]*W1i[...,1] #This is the full rho
    ...
    def J_sep(self,Jg=None,Wg=None,update_eq=0):    
        ...
        for ite_num in range(self.p.max_iter):
            ...
            Wderiv[:, :, 1:-1, :]     = (W1i[:, :, 2:, :, 1] - W1i[:, :, :-2, :, 1]) / (self.N_grid1[ax, ax, 2:, ax] - self.N_grid1[ax, ax, :-2, ax])
            Jderiv0[:, 1:-1, :, :]    = (Ji[:, 2:, :, :] - Ji[:, :-2, :, :]) / (self.N_grid[ax, 2:, ax, ax] - self.N_grid[ax, :-2, ax, ax])
            ...
            if ite_num<=100000000:
             ...
             foc_2ndpart = - self.N_grid1[self.grid[2][:, :, :, ax, :]]*rho_grid[ax, ax, ax, ax, :] -\
            ...
            if ite_num>1:
                sep_star[...] = 0 
                Ifire = (EJinv0 < 0) & (self.N_grid[self.grid[1]]+self.N_grid1[self.grid[2]] <= self.p.n_bar)
            ...
            Ui = 0.4*Ui + 0.6*Ui2
            if ite_num>1:
                #interiorsize = self.N_grid[self.grid[1]] + self.N_grid1[self.grid[2]] <= self.p.n_bar
    ...
    def J_sep_dir(self,Jg=None,Wg=None,update_eq=0):    
        ...
        for ite_num in range(self.p.max_iter):
            ...
            if ite_num>1:
                ...
                EJderiv0 = EJderivative(EJpi,np.floor((self.N_grid[self.grid[1]]+self.N_grid1[self.grid[2]])*pc_star).astype(int),np.ceil((self.N_grid[self.grid[1]]+self.N_grid1[self.grid[2]])*pc_star).astype(int),EJderiv0,rho_grid, self.N_grid1,rho_star,self.p.num_z, self.p.num_n, self.p.n_bar, self.p.num_v)
                EWderiv0 = EWderivative(EW1i,np.floor((self.N_grid[self.grid[1]]+self.N_grid1[self.grid[2]])*pc_star).astype(int),np.ceil((self.N_grid[self.grid[1]]+self.N_grid1[self.grid[2]])*pc_star).astype(int),EWderiv0,rho_grid, self.N_grid1,rho_star,self.p.num_z, self.p.num_n, self.p.n_bar, self.p.num_v)
                EJderiv1 = EJderivative(EJpi,np.floor(self.N_grid1[self.grid[2]]*pc_star).astype(int),np.ceil(self.N_grid1[self.grid[2]]*pc_star).astype(int),EJderiv1,rho_grid, self.N_grid1,rho_star,self.p.num_z, self.p.num_n, self.p.n_bar, self.p.num_v)
                EWderiv1 = EWderivative(EW1i,np.floor(self.N_grid1[self.grid[2]]*pc_star).astype(int),np.ceil(self.N_grid1[self.grid[2]]*pc_star).astype(int),EWderiv1,rho_grid, self.N_grid1,rho_star,self.p.num_z, self.p.num_n, self.p.n_bar, self.p.num_v)
                sep_star[...] = 0 
                Ifire = (EJderiv0+rho_star*(self.N_grid[self.grid[1]]+self.N_grid1[self.grid[2]])*pc_star*EWderiv0 < 0) & (self.N_grid[self.grid[1]]+self.N_grid1[self.grid[2]] <= self.p.n_bar) & (-(EJderiv1+rho_star*self.N_grid1[self.grid[2]]*pc_star*EWderiv1)*pc_star - (EW1_star+re_star-EUi)/ self.pref.inv_utility_1d(self.v_0-self.p.beta*(EUi)) < 0 )
                worker_future_value = np.zeros_like(EW1i)
                for iz in range(self.p.num_z):
                 for in0 in range(self.p.num_n):
                  for in1 in range(self.p.num_n):
                ...
                #assert np.all(rho_star[Ifire]>rho_min)
                #assert np.all(worker_future_value[Ifire] > EUi)
                sepneg = (EJderiv0+rho_star*(self.N_grid[self.grid[1]]+self.N_grid1[self.grid[2]])*pc_star*EWderiv0 < 0) & (sep_star < 0)
                sep_star[sepneg] = 1
